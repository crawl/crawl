...
{
    // Rate limit.
    // XXX: Since m_last_tick_redraw is set at the end, running time of
    //      ::redraw() is extra.
    if (Options.tile_max_fps > 0)
    {
        const float max_fps = Options.tile_max_fps;
        const unsigned int min_ticks =
            static_cast<unsigned int>(1000.0f / max_fps);
