...
int monster::spec_death() const
{
    int sd = 0;

    if (wearing(OBJ_STAVES, STAFF_NECROMANCY))
        sd += 1 + wearing_ego(OBJ_ARMOUR, SPARM_ATTUNEMENT);

    sd += wearing_ego(OBJ_ARMOUR, SPARM_DEATH);

    sd += scan_artefacts(ARTP_ENHANCE_NECRO);

    return sd;
}

int monster::spec_fire() const
{
    int sf = 0;

    if (wearing(OBJ_STAVES, STAFF_FIRE))
        sf += 1 + wearing_ego(OBJ_ARMOUR, SPARM_ATTUNEMENT);

    sf += wearing_ego(OBJ_ARMOUR, SPARM_FIRE);
    sf += wearing_ego(OBJ_ARMOUR, SPARM_PYROMANIA);

    sf += scan_artefacts(ARTP_ENHANCE_FIRE);
...
int monster::spec_cold() const
{
    int sc = 0;

    if (wearing(OBJ_STAVES, STAFF_COLD))
        sc += 1 + wearing_ego(OBJ_ARMOUR, SPARM_ATTUNEMENT);

    sc += wearing_ego(OBJ_ARMOUR, SPARM_ICE);

    sc += scan_artefacts(ARTP_ENHANCE_ICE);
...
int monster::spec_earth() const
{
    int se = 0;

    // Staves
    if (wearing(OBJ_STAVES, STAFF_EARTH))
        se += 1 + wearing_ego(OBJ_ARMOUR, SPARM_ATTUNEMENT);

    se += wearing_ego(OBJ_ARMOUR, SPARM_EARTH);

    se += scan_artefacts(ARTP_ENHANCE_EARTH);
...
int monster::spec_air() const
{
    int sa = 0;

    // Staves
    if (wearing(OBJ_STAVES, STAFF_AIR))
        sa += 1 + wearing_ego(OBJ_ARMOUR, SPARM_ATTUNEMENT);

    sa += wearing_ego(OBJ_ARMOUR, SPARM_AIR);

    sa += scan_artefacts(ARTP_ENHANCE_AIR);
...
int monster::spec_conj() const
{
    int sc = 0;

    if (wearing(OBJ_STAVES, STAFF_CONJURATION))
        sc += 1 + wearing_ego(OBJ_ARMOUR, SPARM_ATTUNEMENT);

    sc += scan_artefacts(ARTP_ENHANCE_CONJ);

    return sc;
}

int monster::spec_hex() const
{
    return scan_artefacts(ARTP_ENHANCE_HEXES);
}

int monster::spec_summ() const
{
    return scan_artefacts(ARTP_ENHANCE_SUMM);
}

int monster::spec_forgecraft() const
{
    return scan_artefacts(ARTP_ENHANCE_FORGECRAFT);
}

int monster::spec_alchemy() const
{
    int sp = 0;

    if (wearing(OBJ_STAVES, STAFF_ALCHEMY))
        sp += 1 + wearing_ego(OBJ_ARMOUR, SPARM_ATTUNEMENT);

    sp += wearing_jewellery(AMU_CHEMISTRY);

    sp += scan_artefacts(ARTP_ENHANCE_ALCHEMY);
...
int monster::spec_tloc() const
{
    return scan_artefacts(ARTP_ENHANCE_TLOC);
}

int monster::spell_enhancement(spell_type spell) const
{
    ...
    if (typeflags & spschool::forgecraft)
        enhanced += spec_forgecraft();

    if (typeflags & spschool::alchemy)
        enhanced += spec_alchemy();
    ...
    enhanced += archmagi();
    enhanced += unrand_equipped(UNRAND_FOLLY);
