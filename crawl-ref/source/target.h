...
class targeter_olgrebs_last_mercy : public targeter_smite
{
public:
    targeter_olgrebs_last_mercy();
    bool set_aim(coord_def a) override;
};
...
class targeter_explosive_bolt : public targeter_beam
{
public:
    targeter_explosive_bolt(const actor* act, int pow, int range);
    bool set_aim(coord_def a) override;
    aff_type is_affected(coord_def loc) override;
private:
...
{
public:
    targeter_cone(const actor *act, int r, double arc = PI / 4);
