...
void monster_teleport(monster* mons, bool instan, bool silent)
{
    ...
    {
        simple_monster_message(*mons, "은(는) 잠시 깜빡거렸다.");
        return;
    }

    if (!silent)
        simple_monster_message(*mons, "이(가) 사라졌다!");
    ...
    {
        if (was_seen)
            simple_monster_message(*mons, "이(가) 근처에 다시 나타났다!");
        else
        {
            ...
            if (!interrupt_activity(AI_SEE_MONSTER, ai))
                simple_monster_message(*mons, "이(가) 갑자기 나타났다!");
        }
    }
