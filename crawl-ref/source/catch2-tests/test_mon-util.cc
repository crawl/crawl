...
TEST_CASE("mons_habitat_type returns correct habitats", "[single-file]") {
    ...
    SECTION("Land monsters should be HT_LAND") {
        const habitat_type habitat = mons_habitat_type(MONS_KOBOLD,MONS_KOBOLD);

        REQUIRE(habitat == HT_LAND);
    }

    SECTION("Lava monsters should be HT_LAVA") {
        const habitat_type habitat = mons_habitat_type(MONS_LAVA_SNAKE,MONS_LAVA_SNAKE);

        REQUIRE(habitat == HT_LAVA);
    }
    SECTION("Water monsters should be HT_WATER") {
        const habitat_type habitat = mons_habitat_type(MONS_KRAKEN,MONS_KRAKEN);

        REQUIRE(habitat == HT_WATER);
    }

    SECTION("Amphibious monsters should be HT_AMPHIBIOUS") {
        const habitat_type habitat = mons_habitat_type(MONS_FRILLED_LIZARD,MONS_FRILLED_LIZARD);
        REQUIRE(habitat == HT_AMPHIBIOUS);
    }

    SECTION("Amphibious (lava) monsters should be HT_AMPHIBIOUS_LAVA") {
        const habitat_type habitat = mons_habitat_type(MONS_SALAMANDER,MONS_SALAMANDER);
        REQUIRE(habitat == HT_AMPHIBIOUS_LAVA);
    }

    SECTION("Zombies of amphibious monsters should still be HT_AMPHIBIOUS") {
        const habitat_type habitat = mons_habitat_type(MONS_ZOMBIE,MONS_FRILLED_LIZARD);
        REQUIRE(habitat == HT_AMPHIBIOUS);
    }

    SECTION("Giants should show up as HT_AMPHIBIOUS unless explicitly told not to") {
        habitat_type habitat = mons_habitat_type(MONS_CYCLOPS,MONS_CYCLOPS);
        REQUIRE(habitat == HT_AMPHIBIOUS);

        habitat = mons_habitat_type(MONS_CYCLOPS,MONS_CYCLOPS,true);
        REQUIRE(habitat != HT_AMPHIBIOUS);
    }

    SECTION("Simulacrum Salamanders should NOT still be HT_AMPHIBIOUS_LAVA") {
        const habitat_type habitat = mons_habitat_type(MONS_SIMULACRUM,MONS_SALAMANDER);
        REQUIRE(habitat != HT_AMPHIBIOUS_LAVA);
    }
}
