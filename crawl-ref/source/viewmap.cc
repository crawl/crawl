...
// Return a description, and also (&c.) if there is something underneath it.
static pair<string, string> _describe_top_thing_in_cell(const coord_def &cpos)
{
    const map_cell &cell = env.map_knowledge(cpos);
    string out, feat, item, etc = " (&c.)";

    if (cell.invisible_monster())
        out = "Something you can't see properly";
    else if (cell.mon_type() != MONS_NO_MONSTER)
        out = get_monster_equipment_desc(*cell.monsterinfo());

    if (cell.cloud() != CLOUD_NONE)
    {
        if (!out.empty())
            return {out, etc};
        out = cloud_type_name(cell.cloud(), false);
    }
...
#endif // WIZARD
    int xwidth = strwidth(extra), allowed = columns - helplen - 1;
    if (xwidth < allowed)
