...
{
    const map_cell& mc = map_knowledge(gc);
...
static bool _feat_has_ink(coord_def gc,
                          const map_knowledge_type& map_knowledge)
{
    return map_knowledge.cloud(gc) == CLOUD_INK;
}
                              ...
                              const map_knowledge_type& map_knowledge)
{
    ...
    const auto feat = map_knowledge(gc).feat();
    const bool feat_has_ink = _feat_has_ink(gc, map_knowledge);
    ...
    for (adjacent_iterator ri(gc, true); ri; ++ri)
    {
        ...
        const bool ink = _feat_has_ink(*ri, map_knowledge);
                                ...
                                const map_knowledge_type& map_knowledge)
{
    ...
    // bit of shoreline.
    auto feat = map_knowledge(gc).feat();
    auto colour = map_knowledge(gc).feat_colour();
...
static void _pack_wall_shadows(const coord_def &gc, packed_cell& cell,
                               ...
                               tileidx_t tile)
{
    if (_is_seen_wall(gc, map_knowledge)
        || _safe_feat(gc, map_knowledge) == DNGN_GRATE)
    {
        return;
    }
                              ...
                              const map_knowledge_type& map_knowledge)
{
    if (!map_bounds(gc))
        return false;
    const map_cell& mc = map_knowledge(gc);
    return feat_is_wall(mc.feat()) && mc.flags & MAP_ICY;
}
                        ...
                        const map_knowledge_type& map_knowledge)
{
    ...
    if (env.level_state & LSTATE_SLIMY_WALL
        && mc.feat() != DNGN_SLIMY_WALL)
    {
        _add_directional_overlays(gc, cell, map_knowledge,
                                  TILE_SLIME_OVERLAY, _is_seen_slimy_wall);
    }
    else if (env.level_state & LSTATE_ICY_WALL
             && mc.flags & MAP_ICY)
    {
        if (feat_is_wall(mc.feat()))
        {
    ...
    else
    {
        ...
        _pack_wall_shadows(gc, cell, map_knowledge, shadow_tile);
    }
}
#endif //TILECELL.CC
