    ...
    function minimap_farview(ev)
    {
        if (ev.which == 3 || farview_old_vc)
        {
            ...
            if (farview_old_vc === undefined)
            {
                ...
                farview_old_map_cursor = view_data.cursor_locs[CURSOR_MAP];
            }
            ...
            var y = Math.round((ev.pageY - display_y - offset.top)
                               / cell_h + cell_y - 0.5);
            vgrdc(x, y);
            view_data.place_cursor(enums.CURSOR_MAP, x, y);
        }
    }
    ...
    function stop_minimap_farview(ev)
    {
        if (farview_old_vc !== undefined)
        {
            ...
            if (farview_old_map_cursor)
                view_data.place_cursor(enums.CURSOR_MAP, farview_old_map_cursor.x,
