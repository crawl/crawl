...
function receive_message(data) {
  ...
  const atBottom =
    Math.abs(histcon.scrollHeight - histcon.scrollTop - histcon.clientHeight) <
    1.0;
  msg.find(".chat_msg").html(linkify(msg.find(".chat_msg").text()));
  $("#chat_history").append(`${msg.html()}<br>`);
  if (atBottom) histcon.scrollTop = histcon.scrollHeight;
...
function handle_dump(data) {
  const msg = $("<span>")
    .addClass("chat_msg chat_automated")
    .text(`${data.url}.txt`);
  receive_message({ content: msg[0].outerHTML });
}

function update_message_count() {
  const press_key = new_message_count > 0 ? " (Press F12)" : "";
...
function chat_message_send(e) {
  ...
  // Up arrow to access message history.
    ...
    e.preventDefault();
    e.stopPropagation();
    const lim = Math.min(message_history.length, history_limit);
...
function toggle_entire_chat() {
  if ($("#chat").css("display") === "none") {
    ...
    if ($("#chat_body").css("display") === "none") $("#chat").show();
    else $("#chat").slideDown(200);
  } else {
    chat_hidden = true;
    $(document.activeElement).blur();
    // slide looks odd if the chat box is already 1 line
    if ($("#chat_body").css("display") === "none") $("#chat").hide();
    else $("#chat").slideUp(200);
  }
...
function super_hide_chat() {
  ...
  if ($("#chat").css("display") !== "none") toggle_entire_chat();
  $("#chat_hidden").remove();
}
...
function focus() {
  if (!$("#chat_hidden").length) return;
  if ($("#chat_hidden").css("display") !== "none") toggle_entire_chat();
  if ($("#chat_body").css("display") === "none") toggle();
...
export default { spectators, clear, focus, reset_visibility, show_in_chat };
