...
let showing_close_message = false;
let current_hash;
...
window.log = (text) => {
  if (window.console?.log) window.console.log(text);
};

function handle_message(msg) {
  if (typeof msg === "string") {
    // Javascript code
    // biome-ignore lint/security/noGlobalEval: necessary for sockets
    eval(msg);
  } else {
...
function handle_message_backlog() {
  while (message_queue.length && message_inhibit === 0) {
...
function uninhibit_messages() {
  if (message_inhibit > 0) message_inhibit--;
  handle_message_backlog();
}
...
const layers = ["crt", "normal", "lobby", "loader"];

function in_game() {
  return (
    current_layer !== "lobby" &&
    current_layer !== "loader" &&
...
function do_layout() {
  ...
  if (window.do_layout) window.do_layout();
}
...
function handle_keypress(e) {
  ...
  if (
    ...
    /* Tab gives a keypress in Opera even when it is
                                 suppressed in keydown */
    return;
  }

  // Give the game a chance to handle the key
  if (!retrigger_event(e, "game_keypress")) return;
  ...
  } else send_message("input", { text: s });
}
...
function handle_keydown(e) {
  ...
  if ($("#force_terminate").is(":visible")) {
    e.preventDefault();
    if (e.which === "y".charCodeAt(0)) force_terminate_yes();
    else force_terminate_no();
    return;
  }
  ...
  // though.
  const code = e.originalEvent.code ? e.originalEvent.code : e.code;
  ...
  // Give the game a chance to handle the key
  if (!retrigger_event(e, "game_keydown")) return;
  ...
  if (e.ctrlKey && !e.shiftKey && !e.altKey) {
    if (e.which in key_conversion.ctrl) {
      ...
      e.preventDefault();
      const code = e.which - "A".charCodeAt(0) + 1; // Compare the CONTROL macro in defines.h
      send_keycode(code);
    }
    ...
    if (e.which === 123 || (code && code === "F12")) {
      // probably unncessesary for now
      e.preventDefault();
      chat.focus();
    } else if (
      key_conversion.codes && // <- cache safety, remove someday
      key_conversion.code_conversion_enabled() &&
      code &&
      code in key_conversion.codes
    ) {
      e.preventDefault();
...
function auth_error(data) {
  ...
  if (reason) $("#login_message").html(reason);
  else $("#login_message").html("Login failed.");
}
...
function logged_in(data) {
  ...
  if (admin_user) $("#admin_panel_button").show();
  else $("#admin_panel_button").hide();
...
function set_login_cookie(data) {
  if (data) $.cookie("login", data.cookie, { expires: data.expires });
  else $.cookie("login", null);
}
...
function toggle_admin_panel() {
  if (admin_user) $("#admin_panel").toggle();
}

function admin_announce() {
  const text = $("#announcement_text").val();
  if (text.length > 0) {
...
function admin_pw_reset() {
  ...
  if (username.length === 0) admin_log_msg("No username provided!");
  else send_message("admin_pw_reset", { username: username });
}

function admin_pw_reset_done(data) {
  if (data.error) admin_log_msg(`Password reset failed: ${data.error}`);
  else {
...
function admin_log_msg(text) {
  $("#admin_panel_log").append(`<div><span>${text}</span></div>`);
}
...
function server_announcement(data) {
  const text =
    '<span class="fg5">Serverwide announcement: </span><span>' +
    data.text +
    "</span>";
...
function show_dialog(id) {
  ...
  const elem = $(id);
  elem.stop(true, true).fadeIn(100, () => {
    elem.focus();
  });
  center_element(elem);
  $("#overlay").show();
...
function center_element(elem) {
  let left = $(window).width() / 2 - elem.outerWidth() / 2;
  if (left < 0) left = 0;
  let top = $(window).height() / 2 - elem.outerHeight() / 2;
  if (top < 0) top = 0;
  if (top > 50) top = 50;
...
function handle_dialog(data) {
  const dialog = $("<div class='floating_dialog'>");
  dialog.html(data.html);
  $("body").append(dialog);
  dialog.find("[data-key]").on("click", function (_ev) {
...
function exit_reason_message(reason, watched_name) {
  if (watched_name) {
    ...
    switch (reason) {
      ...
      default:
        return (
          "Unfortunately your game ended unexpectedly." +
          (reason !== "unknown" ? ` (${reason})` : "")
        );
    }
  }
...
function show_exit_dialog(reason, message, dump, watched_name) {
  const reason_msg = exit_reason_message(reason, watched_name);
  if (reason_msg != null) $("#exit_game_reason").text(reason_msg).show();
  else $("#exit_game_reason").hide();

  if (message) $("#exit_game_message").text(message.replace(/\s+$/, "")).show();
  else $("#exit_game_message").hide();

  if (dump) {
    const a = $("<a>").attr("target", "_blank").attr("href", `${dump}.txt`);
    if (reason === "saved") a.text("Character dump");
    else if (reason === "crash") a.text("Crash log");
    else a.text("Morgue file");
    $("#exit_game_dump").html(a).show();
  } else $("#exit_game_dump").hide();
...
function register() {
  ...
  const password_repeat = $("#reg_repeat_password").val();
...
function start_change_password(_data) {
  $("#chpw_message").html("");
...
function change_password_done(_data) {
  $("#ok_message_content").html("Password changed!");
  show_dialog("#floating_ok_message");
}

function change_password() {
  const cur_password = $("#change_cur_password").val();
  const new_password = $("#change_new_password").val();
  const new_password_repeat = $("#change_repeat_password").val();
...
function change_password_failed(data) {
  ...
  if (data.reason) msg = `${msg} ${data.reason}`;
...
function reset_password() {
  const token = $("#reset_pw_token").val();
  const password = $("#reset_pw_password").val();
  const password_repeat = $("#reset_pw_repeat_password").val();
...
function go_lobby() {
  ...
  if (exit_reason) {
    if (
      was_watching ||
      normal_exit.indexOf(exit_reason) === -1 ||
...
function chat_login(_data) {
  if (!in_game() || !watching) return;

  const a = $("<a href='javascript:'>Close</a>");
  a.click(hide_prompt);
...
function lobby_entry(data) {
  ...
  const username_entry = $(make_watch_link(data));
  ...
  if (data.turn && data.dur) {
    set("turn", data.turn);
    set("dur", format_duration(parseInt(data.dur, 10)));
  ...
  if (entry.find(".milestone").text() !== data.milestone) {
    if (single) roll_in_new_milestone(entry, data.milestone);
    else set("milestone", data.milestone);
  }

  if (single) lobby_complete();
}

const _lobby_idle_timer = setInterval(update_lobby_idle_times, 1000);
function update_lobby_idle_times() {
  $("#player_list .idle_time").each(function () {
    ...
    if (time) {
      ...
      $this.data("time", time).attr("data-time", `${time}`);
      $this.data("sort", `${time}`).attr("data-sort", `${time}`);
    }
  });
}
...
function lobby_remove(data) {
  $(`#game-${data.id}`).remove();
  if ($("#player_list tbody tr").length === 0) $("#lobby_body").hide();
}
...
function lobby_complete() {
  const old_list = $("#player_list");
  let sortlist;
  if (new_list.find("tbody tr").length > 0) {
    $("#lobby_body").show();
    if (old_list[0].config) sortlist = old_list[0].config.sortList;
    else sortlist = [[0, 0]];
...
function make_watch_link(data) {
  ...
  else return `<a href='#watch-${data.username}'></a>`;
}
...
function extract_text_or_data(elem) {
  const $elem = $(elem);
  if ($elem.data("sort")) return $elem.data("sort");
  else return $elem.text();
}

function roll_in_new_milestone(row, milestone) {
  const td = row.find(".milestone_col");
  if (td.length === 0) return;

  const new_milestone = td.find(".new_milestone");
  new_milestone.text(milestone);

  const milestones = td.find(".new_milestone, .milestone");
  milestones.animate({ top: "-1.1em" }, () => {
...
function handle_force_terminate(_data) {
...
function hash_changed() {
  if (location.hash === current_hash) return;
  current_hash = location.hash;

  const watch = location.hash.match(/^#watch-(.+)/i);
  const play = location.hash.match(/^#play-(.+)/i);
  if (watch) {
    const watch_user = watch[1];
    send_message("watch", {
      username: watch_user,
    });
  } else if (play) {
    const game_id = play[1];
    send_message("play", {
      game_id: game_id,
    });
  } else {
...
function receive_game_client(data) {
  if (game_version == null || data.version !== game_version) {
    ...
    for (const i in loaded_modules) requirejs.undef(loaded_modules[i]);
    loaded_modules = [];
  }
  if (game_script) {
    document.head.removeChild(game_script);
  }
  ...
  const waitForImages = () => {
    $("#game").waitForImages(() => {
      const load_interval = setInterval(check_loading, 50);
  ...
  if (data.content.indexOf("<!-- DCSS.WebTiles.v2 -->") >= 0) {
    ...
    window.DCSS ??= {};
    window.DCSS.comm = comm;
    window.DCSS.client = client;
    window.DCSS.key_conversion = key_conversion;
...
function decode_utf8(bufs, callback) {
  if (text_decoder) callback(text_decoder.decode(bufs));
  else {
...
let blob_construction_supported = true;
try {
  const _blob = new Blob([new Uint8Array([0])]);
} catch (_e) {
...
function inflate_works_on_ua() {
  if (!blob_construction_supported) return false;
  const b = $.browser;
  if (b.chrome && b.version.match("^14.")) return false; // doesn't support binary frames
  if (b.chrome && b.version.match("^19.")) return false; // buggy Blob builder
  if (b.safari) return false;
  if (b.opera) return false; // errors in inflate.js (?)
  return true;
}
...
$(document).ready(() => {
  ...
  $(window).bind("beforeunload", (ev) => {
    if (location.hash.match(/^#play-(.+)/i) && socket.readyState === 1) {
      ev.preventDefault();
      ev.returnValue = "";
  ...
  if ("WebSocket" in window) {
    ...
    if (inflater) socket = new WebSocket(socket_server);
    else socket = new WebSocket(socket_server, "no-compression");
    ...
    socket.onmessage = (msg) => {
      if (inflater && msg.data instanceof ArrayBuffer) {
        ...
        const decompressed = inflater.append(data);
        if (decompressed === -1) {
          console.error("Decompression error!");
          const _x = inflater.append(data);
        }
        decode_utf8(decompressed, (s) => {
          if (window.log_messages === 2) console.log(`Message: ${s}`);
      ...
      if (window.log_messages === 2) console.log(`Message: ${msg.data}`);
      if (window.log_message_size)
...
client.inhibit_messages = inhibit_messages;
client.uninhibit_messages = uninhibit_messages;
client.delay = delay;
client.in_game = in_game;
client.set_layer = set_layer;
client.register_layer = register_layer;
client.show_dialog = show_dialog;
client.hide_dialog = hide_dialog;
client.center_element = center_element;
client.is_watching = is_watching;
