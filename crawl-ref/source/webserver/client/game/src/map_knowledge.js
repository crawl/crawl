...
let k, player_on_level, monster_table, dirty_locs, bounds, bounds_changed;
...
function touch(x, y) {
  const pos = y === undefined ? x : { x, y };
  const cell = get(pos.x, pos.y);
  if (!cell.dirty) {
    dirty_locs.push(pos);
    cell.dirty = true;
  }
...
function merge_objects(current, diff) {
  ...
  for (const prop in diff) current[prop] = diff[prop];
...
function merge_monster(old_mon, mon) {
  ...
  let last = monster_table[id];
  if (!last) {
    if (old_mon) last = merge_objects(merge_objects({}, old_mon), mon);
    else {
      last = mon;
...
function clean_monster_table() {
  for (const id in monster_table) {
    if (!monster_table[id].refs) delete monster_table[id];
  }
}

let merge_last_x, merge_last_y;

function merge(val) {
  if (val === undefined) return;

  let x, y;
  if (val.x === undefined) x = merge_last_x + 1;
  else x = val.x;
  if (val.y === undefined) y = merge_last_y;
  else y = val.y;
  merge_last_x = x;
  merge_last_y = y;
...
export default {
  ...
  touch: touch,
  visible: visible,
  player_on_level: () => player_on_level,
  set_player_on_level: (v) => {
    player_on_level = v;
  },
  dirty: () => dirty_locs,
  reset_dirty: () => {
