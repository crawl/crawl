...
$.extend(DungeonViewRenderer.prototype, {
  ...
  new_renderer: function (tiles) {
    ...
    const canvas = $("<canvas>");
    renderer.set_cell_size(this.cell_width, this.cell_height);
  ...
  update_mouse_mode: function (_m) {
    if (!game.can_target()) this.last_sent_cursor = { x: 0, y: 0 };
  },
});

const renderer = new DungeonViewRenderer();
...
$(document).on("game_init.dungeon_renderer", () => {
  renderer.cols = 0;
  renderer.rows = 0;
...
$(document)
  .off("game_cleanup.dungeon_renderer")
  .on("game_cleanup.dungeon_renderer", () => {
    if (anim_interval) {
      clearTimeout(anim_interval);
      anim_interval = null;
    }
...
$(document)
  .off("text_update.dungeon_renderer")
  .on("text_update.dungeon_renderer", () => {
    renderer.animate();
  });
