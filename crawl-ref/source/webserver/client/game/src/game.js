...
function setup_keycodes() {
  ...
  // TODO: remove someday
  if (key_conversion.codes) key_conversion.enable_code_conversion();
  else {
...
function layout_params_differ(old_params, new_params) {
  ...
  for (const param in new_params) {
    if (
      Object.hasOwn(old_params, param) &&
...
function layout(params, force) {
  ...
  const state = ui_state;
  set_ui_state(enums.ui.NORMAL);

  // Determine width of stats area
  const old_html = $("#stats").html();
  let s = "";
  for (let i = 0; i < enums.stat_width; i++) s = `${s}&nbsp;`;
  $("#stats").html(s);
  const stat_width_px = $("#stats").outerWidth();
  $("#stats").html(old_html);
  ...
  const old_messages = $("#messages").clone(true);
  const old_scroll_top = $("#messages_container").scrollTop();
  s = "";
  for (let i = 0; i < msg_height + 1; i++) s = `${s}<br>`;
  $("#messages").html(s);
  const msg_height_px = $("#messages").outerHeight();
  ...
  const remaining_height = window_height - msg_height_px;
  ...
  dungeon_renderer.fit_to(remaining_width, remaining_height, show_diameter);
  ...
  const possible_input = $("#messages .game_message input");
  if (possible_input) possible_input.focus();

  // Input helper for mobile browsers
  // XX should this really happen in `layout`?
  if (!client.is_watching()) {
    const mobile_input = options.get("tile_web_mobile_input_helper");
    if (mobile_input === "true" || (mobile_input === "auto" && is_mobile())) {
      ...
      $(document).on("visibilitychange", (_ev) => {
...
options.add_listener(() => {
  ...
  if (layout_parameters) layout(layout_parameters, true);
  display.invalidate(true);
  display.display();
...
function toggle_full_window_dungeon_view(full) {
  ...
  if (full) {
    let width = layout_parameters.remaining_width;
    let height = layout_parameters.remaining_height;
...
function handle_set_layout(data) {
  if (data.message_pane.height) {
    msg_height =
      data.message_pane.height + (data.message_pane.small_more ? 0 : -1);
    messages.message_pane_height = msg_height;
  } else msg_height = messages.message_pane_height;

  if (layout_parameters == null) layout({});
  else {
    const params = $.extend({}, layout_parameters);
    layout(params, true);
  }
}
...
function handle_set_ui_cutoff(data) {
  const popups = document.querySelectorAll("#ui-stack > .ui-popup");
  Array.from(popups).forEach((p, i) => {
...
function set_input_mode(mode) {
  ...
  if (mode === enums.mouse_mode.COMMAND) messages.new_command();
}

game.get_input_mode = () => input_mode;
game.get_ui_state = () => ui_state;

game.can_target = () =>
  input_mode === enums.mouse_mode.TARGET ||
  input_mode === enums.mouse_mode.TARGET_DIR ||
  input_mode === enums.mouse_mode.TARGET_PATH;
game.can_move = () => {
  // XX just looking
  return (
    input_mode === enums.mouse_mode.COMMAND || ui_state === enums.ui.VIEW_MAP
  );
};
game.can_describe = () =>
  input_mode === enums.mouse_mode.TARGET ||
...
function init_custom_text_colours() {
  ...
  // Reset colours first
  for (let i = 0; i < 16; i++) root.style.removeProperty(`--color-${i}`);

  // Load custom replacements
  const colours = options.get("custom_text_colours");
  for (const i in colours) {
    root.style.setProperty(
      `--color-${colours[i].index}`,
      "rgba(" +
        colours[i].r +
        ", " +
        colours[i].g +
