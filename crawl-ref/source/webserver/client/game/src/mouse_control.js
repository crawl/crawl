...
function show_tooltip(text, x, y) {
  $("#tooltip").html(text).css({ left: x, top: y }).show();
}
...
function handle_cell_tooltip(ev) {
  const map_cell = map_knowledge.get(ev.cell.x, ev.cell.y);
  ...
  if (game.can_describe()) {
    ...
    if (text) text += "<br>"; // XX something better than <br>s
...
$(window)
  ...
  .on("mousemove.mouse_control mousedown.mouse_control", (_ev) => {
    hide_tooltip();
  });
