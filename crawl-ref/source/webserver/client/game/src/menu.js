...
function item_selectable(item) {
  ...
  return (
    ...
    (menu.tag === "use_item" || item.hotkeys?.length)
  );
}
...
function item_colour(item) {
  if (item.colour === undefined) return 7;
  return item.colour;
}

function menu_title_indent() {
  if (
    !options.get("tile_menu_icons") ||
...
let menu_close_timeout = null;
let mouse_hover_suppressed = null;

function add_hover_class(item) {
  if (item < 0 || item >= menu.items.length) return;
...
function mouse_set_hovered(index) {
  if (mouse_hover_suppressed) return;
  if (
    index >= 0 &&
...
function suppress_mouse_hover() {
  ...
  if (mouse_hover_suppressed) clearTimeout(mouse_hover_suppressed);
...
function display_menu() {
  const menu_div = $(".templates > .menu").clone();
  ...
  const content_div = $("<div class='menu_contents'>");
  menu_div.append(content_div);

  const items_inner = $("<div class='menu_contents_inner'>");
  content_div.append(items_inner);

  const container = $("<ol>");
  items_inner.append(container);

  const chunk = menu.items;
  ...
  if (client.is_watching()) menu.following_player_scroll = true;
  ...
  $(ui.top_popup()).on("focusin", (_ev) => {
    suppress_mouse_hover();
  });

  if (menu.flags & enums.menu_flag.START_AT_END)
    scroll_bottom_to_item(menu.items.length - 1, true);
  else if (menu.jump_to) scroll_to_item(menu.jump_to, true);
  else if (menu.items.length > 0) scroll_to_item(0, true);

  if (menu.last_hovered >= 0) add_hover_class(menu.last_hovered);
}
...
function prepare_item_range(start, end, container) {
  ...
  if (start < 0) start = 0;
  if (end >= menu.total_items) end = menu.total_items - 1;

  if (start >= menu.total_items || end < 0) return;
  ...
  // home and end.
  while (menu.items[start] !== undefined && start <= end) start++;
  if (start > end) return;
  while (menu.items[end] !== undefined && start <= end) end--;

  container = container || menu.elem.find(".menu_contents_inner ol");

  // Find the place where we add the new elements
  let present_index = end;
  while (
    present_index < menu.total_items &&
    menu.items[present_index] === undefined
  ) {
    present_index++;
  }
  let anchor = null;
  if (menu.items[present_index]) anchor = menu.items[present_index].elem;
  ...
  for (let i = start; i <= end; ++i) {
    ...
    if (anchor) anchor.before(elem);
    else container.append(elem);

    menu.items[i] = item;
  }

  if (start < menu.first_present) menu.first_present = start;
  if (end > menu.last_present) menu.last_present = end;
}

function update_item_range(chunk_start, items_list) {
  prepare_item_range(0, menu.total_items - 1);
  for (let i = 0; i < items_list.length; ++i) {
    const real_index = i + chunk_start;
    const item = menu.items[real_index];
    if (!item) continue;
    let new_item = items_list[i];
    ...
    if (new_item.colour === undefined) delete item.colour;
    if (new_item.tiles === undefined) delete item.tiles;
    if (new_item.hotkeys === undefined) delete item.hotkeys;
...
function next_hoverable_item(
  ...
  menu.flags;
  if (menu.flags & enums.menu_flag.WRAP) max_items = menu.items.length;
  else if (reverse)
    max_items = menu.last_hovered; // up arrow on no hover does nothing
  else max_items = menu.items.length - Math.max(starting_point, 0);
  ...
  let new_hover = starting_point;
  if (reverse && new_hover < 0) new_hover = 0;
  let found = false;
  if (!start_at_starting_point) new_hover = new_hover + (reverse ? -1 : 1);
  ...
  while (items_tried < max_items) {
    ...
    if (item_selectable(menu.items[new_hover])) {
      // TODO: not identical to c++ logic
      found = true;
      break;
    }
...
function cycle_hover(reverse) {
  const next = next_hoverable_item(reverse, menu.last_hovered);
  if (next !== -1) set_hovered(next);
}
...
function get_relative_hover() {
  ...
  // generalized?
  if (menu.last_visible === menu.last_hovered) return -1;

  let relative_hover = menu.last_hovered - menu.first_visible;

  let headers = 0;

  // factor out headers
  for (let i = 0; i <= relative_hover; i++)
    if (menu.items[menu.first_visible + i].level < 2) headers += 1;
...
function set_relative_hover(relative_hover) {
  ...
  if (relative_hover >= 0) {
    ...
    for (
      let i = 0;
      i <= relative_hover && menu.first_visible + i <= menu.last_visible;
      i++
    ) {
      if (menu.items[menu.first_visible + i].level < 2) relative_hover += 1;
    }

    hover_target = menu.first_visible + relative_hover;
  } // XX headers not handled
  else hover_target = menu.last_visible + relative_hover + 1;

  // don't scroll -- lock to whatever is currently visible
  if (hover_target > menu.last_visible) hover_target = menu.last_visible;
  else if (hover_target < menu.first_visible) hover_target = menu.first_visible;
...
function paging(up) {
  ...
  const adjust_hover = menu.last_hovered >= 0;
  let relative_hover = get_relative_hover();
  if (up) {
    // if we are already at the top, don't scroll, and move any hover
    // to the top
    if (menu.first_visible === 0) relative_hover = 0;
    else scroll_bottom_to_item(menu.first_visible);
  } else {
    // if we are already at the end, don't scroll, and move any hover
    // to the end
    if (menu.last_visible === menu.items.length - 1) relative_hover = -1;
    else scroll_to_item(menu.last_visible);
  }

  if (adjust_hover) set_relative_hover(relative_hover);
}

function line_down() {
  if (menu.length <= 0) return;
  let next = menu.first_visible + 1;
  // treat a header and a following item as one item
  if (menu.items[menu.first_visible].level < 2) next = next + 1;
  if (next >= menu.items.length) next = menu.items.length - 1;
  scroll_to_item(next);
  snap_hover_in_page();
}

function line_up() {
  let previous = menu.first_visible - 1;
...
function snap_in_page(index) {
  ...
  if (index <= menu.first_visible) scroll_to_item(index);
  else if (index >= menu.last_visible) scroll_bottom_to_item(index);
}

function scroll_to_item(item_or_index, was_server_initiated) {
  const _index = item_or_index.elem ? item_or_index.index : item_or_index;
  if (menu.items.length === 0) return;

  let item = item_or_index.elem ? item_or_index : menu.items[item_or_index];
  ...
  if (item.index === menu.first_visible) return;
  const contents = $(menu.scroller.scrollElement);
  const baseline = contents.children()[0].getBoundingClientRect().top;
  const elem_y = item.elem[0].getBoundingClientRect().top;
  if (menu.flags & enums.menu_flag.ARROWS_SELECT) suppress_mouse_hover();
...
function scroll_bottom_to_item(item_or_index, was_server_initiated) {
  const index = item_or_index.elem ? item_or_index.index : item_or_index;
  if (menu.items.length === 0 || menu.last_visible === index) return;

  const item = item_or_index.elem ? item_or_index : menu.items[item_or_index];
  const contents = $(menu.scroller.scrollElement);
  const baseline = contents.children().offset().top;

  if (menu.flags & enums.menu_flag.ARROWS_SELECT) suppress_mouse_hover();

  // allow a bit of extra space for the fade, number may need more tuning
  contents.scrollTop(
    item.elem.offset().top +
      item.elem.height() +
      24 -
      baseline -
...
function update_visible_indices() {
  const $contents = menu.elem.find(".menu_contents");
  const container_rect = $contents.children()[0].getBoundingClientRect();
  ...
  for (i = 0; i < menu.items.length; i++) {
    ...
    if (item_rect.top + 1.0 >= top) {
      ...
      break;
    } else if (item_rect.bottom - top > 10)
      // XX 10 here is a bit heuristic
  ...
  for (; i < menu.items.length; i++) {
    ...
    if (item_rect.bottom - 1.0 >= bottom) {
      ...
      if (bottom - item_rect.top > 10) menu.last_part_visible = i;
      break;
    } else menu.last_visible = i;
  }
  if (menu.first_part_visible === -1)
    menu.first_part_visible = menu.first_visible;
  if (menu.last_part_visible === -1) menu.last_part_visible = menu.last_visible;
  update_more();
}
...
function update_title() {
  const title = menu.elem.find(".menu_title");
...
function title_prompt(data) {
  ...
  const restore = () => {
    if (!client.is_watching || !client.is_watching()) $("#title_prompt").blur();
  ...
  if (!data || !data.prompt) prompt = "Select what? (regex) ";
...
function close_all_menus() {
  while (menu_stack.length > 0) close_menu();
}
...
function update_more() {
  if (menu.type !== "crt") {
    ...
    let shown_more = contents_height > avail_height ? menu.more : menu.alt_more;

    const scroll_end = contents_height - avail_height;
    let scroll_percent;
    if (contents[0].scrollTop === 0 || scroll_end <= 0) scroll_percent = "top";
    ...
    const more = menu.elem.find(".menu_more");
...
function server_menu_scroll(data) {
  if (!data.force && !client.is_watching()) return;
...
function handle_size_change() {
  ...
  if (menu.last_hovered > menu.items.length) menu.last_hovered = -1; // sanity check
  // ensure hover class is properly set.
  if (
    menu.last_hovered >= 0 &&
    !item_selectable(menu.items[menu.last_hovered])
  ) {
    // a mildly smarter behavior would find the first hoverable position
    // ignoring headers
    cycle_hover(false);
  } else set_hovered(menu.last_hovered);

  if (menu.anchor_last) scroll_bottom_to_item(menu.last_visible, true);
  else if (menu.first_visible) scroll_to_item(menu.first_visible, true);
  else update_visible_indices();
...
function menu_scroll_handler(was_server_initiated) {
  ...
  if (was_server_initiated === false) menu.following_player_scroll = false;
...
function raw_arrow_keys() {
  return (
    menu.tag === "macro_mapping" &&
    ($(".raw_input_mode").length || // raw input mode is up
      menu.items.length === 0)
  ); // new binding input mode
}
...
function menu_keydown_handler(event) {
  ...
  switch (event.which) {
    ...
    case 109: // numpad -
      if (menu_has_custom_dash()) break;
    // otherwise, fall through to pageup:
    case 33: // page up
      if (raw_arrow_keys()) break; // Treat input as raw, no need to scroll anyway
      paging(true);
      event.preventDefault();
      return false;
    case 107: // numpad +
    case 34: // page down
      if (raw_arrow_keys()) break;
      paging();
      event.preventDefault();
      return false;
    case 35: // end
...
function menu_has_custom_dash() {
  return (
    menu.tag === "inventory" || // drop/pickup use '-' to clear all (TODO: specific tag?)
    menu.tag === "stash" || // TODO: ??
...
function menu_keypress_handler(event) {
  ...
  if (
    chr === " " &&
    menu.flags & enums.menu_flag.MULTISELECT &&
...
function item_click_handler(event) {
  ...
  if (menu.flags & enums.menu_flag.ARROWS_SELECT) {
    ...
    if (menu.flags & enums.menu_flag.SINGLESELECT) {
      if (event.which === 1) comm.send_message("key", { keycode: 13 });
      else if (event.which === 3) comm.send_message("input", { text: "'" });
    } else if (menu.flags & enums.menu_flag.MULTISELECT) {
      if (event.which === 1) comm.send_message("key", { keycode: 32 });
      else if (event.which === 3) comm.send_message("input", { text: "'" });
    }
  }
  // TODO: don't rely on hotkeys here
  else if (item.hotkeys?.length && event.which === 1)
...
options.add_listener(() => {
  if (options.get("tile_font_crt_size") === 0) {
    ...
    $("#crt").css("font-size", `${options.get("tile_font_crt_size")}px`);
    $(".menu").css("font-size", `${options.get("tile_font_crt_size")}px`);
  }
...
$(document)
  ...
  .on("game_init.menu", () => {
    menu_stack = [];
    $(window).off("resize.menu").on("resize.menu", handle_size_change);
    $(document)
      .off("game_keydown.menu game_keypress.menu")
      .on("game_keydown.menu", menu_keydown_handler)
      .on("game_keypress.menu", menu_keypress_handler);
    $(document).off("game_cleanup_menu").on("game_cleanup.menu", menu_cleanup);
  });
