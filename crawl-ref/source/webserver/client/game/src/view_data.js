...
exports.get_flash_colour = (flash_colour, flash_alpha) => {
  if (!flash_colour) return null;
  const colour = flash_colours[flash_colour];

  if (flash_alpha === undefined) flash_alpha = 0;

  if (flash_alpha === 0) return colour;
...
function place_cursor(type, loc) {
  ...
  if (old_loc) rerender.push(old_loc);

  if (loc) rerender.push(loc);

  $("#dungeon").trigger("update_cells", [rerender]);
}

exports.place_cursor = place_cursor;
exports.remove_cursor = (type) => {
