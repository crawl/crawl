...
function prepare_flags(tileidx, flagdata, cache) {
  if (typeof tileidx === "number") tileidx = [tileidx];
  else if (tileidx.value !== undefined) return tileidx;
  while (tileidx.length < 2) tileidx.push(0);

  if (cache[[tileidx[0], tileidx[1]]] !== undefined)
    return cache[[tileidx[0], tileidx[1]]];

  for (const flagname in flagdata.flags) {
    const flagmask = flagdata.flags[flagname];
    if (Array.isArray(flagmask))
      tileidx[flagname] = array_nonzero(array_and(tileidx, flagmask));
    else tileidx[flagname] = (tileidx[0] & flagmask) !== 0;
  }

  for (let i = 0; i < flagdata.exclusive_flags.length; ++i) {
    const excl = flagdata.exclusive_flags[i];
    let val;
    if (Array.isArray(excl.mask)) val = array_and(tileidx, excl.mask);
    else val = [tileidx[0] & excl.mask];

    for (const flagname in excl) {
      if (flagname === "mask") continue;
      if (Array.isArray(excl[flagname]))
        tileidx[flagname] = array_equal(val, excl[flagname]);
      else tileidx[flagname] = val[0] === excl[flagname];
    }
  }
...
const fg_flags = { flags: {}, exclusive_flags: [] };
...
// Background flags
const bg_flags = { flags: {}, exclusive_flags: [] };
...
let fg_cache = { size: 0 };
exports.prepare_fg_flags = (tileidx) => {
  if (fg_cache.size >= 100) fg_cache = { size: 0 };
  return prepare_flags(tileidx, fg_flags, fg_cache);
};
let bg_cache = { size: 0 };
exports.prepare_bg_flags = (tileidx) => {
  if (bg_cache.size >= 250) bg_cache = { size: 0 };
...
exports.reverse_lookup = (e, value) => {
  for (const prop in e) {
    if (e[prop] === value) return prop;
  }
};
