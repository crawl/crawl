...
void orb_pickup_noise(const coord_def& where, int loudness, const char* msg, const char* msg2)
{
    ...
    {
        if (msg)
            mprfc(MSGCH_ORB, "%s", msg);
        else
            mprfc(MSGCH_ORB, "The Orb lets out an agonising shriek!");
    }
    else
    {
        if (msg2)
            mprfc(MSGCH_ORB, "%s", msg2);
        else
            mprfc(MSGCH_ORB, "The Orb lets out a furious burst of light!");
    }
}
...
void start_orb_run(game_chapter chapter, const char* message)
{
    ...
    {
        mprfc(MSGCH_WARN, "The lords of Pandemonium are not amused. Beware!");
        if (have_passive(passive_t::slow_orb_run))
            simple_god_message(" tells them not to hurry.");
    }

    mprfc(MSGCH_ORB, "%s", message);
    you.chapter = chapter;
