...
function TroveMarker:fdesc_long (marker)
  ...
  if self.no_timeout then
    state = "This portal is broken and requires " .. self:item_name() .. " to function.\n"
  else
    state = "This portal will remain here for a" .. self.props.dur .. " time.\n"
  end
...
function TroveMarker:check_veto(marker, pname)
  ...
  if pname == "veto_stair" then
    ...
    if self.no_timeout == true then
      ...
      if crawl.yesno("Pay? (you can refuse and instead pay the full fee of " .. needed .. "gp to enter now)", true, "n") then
        self.no_timeout = true
        you.gold(you.gold() - needed2)
        self:note_payed(needed2 .. "gp", true)
        crawl.mpr("The imp fiddles with the portal and frowns. It says, \"I think I broke it! You might be able to fix it with " .. self:item_name() .. ".\" before disappearing in a puff of smoke.")
        return "veto"
      else
