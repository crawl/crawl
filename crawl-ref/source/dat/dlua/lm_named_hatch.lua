------------------------------------------------------------------------------
-- lm_named_hatch.lua:
-- Hatches with fixed destinations on their target level.
--
-- Set a lua marker on the escape hatch using hatch_name(name), and set another
-- corresponding marker on the hatch destination on the destination using the
-- same string in `name'. When the player takes this hatch, the chosen
-- destination will be that of marked destination instead of a random
-- destination.
------------------------------------------------------------------------------
