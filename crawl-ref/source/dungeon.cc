...
void place_spec_shop(const coord_def& where, shop_spec &spec, int shop_level)
{
    ...
    if (shop.type == SHOP_RANDOM)
    {
        ...
        // Re-roll if we got a gadget shop below level 10; you need to roll it twice
        // in a row to get one here. Level 10 means they start showing up in Orc.
        // Re-roll half the time elsewhere so they're still a bit rare.
        if (shop.type == SHOP_GADGET && (level_number < 10 || coinflip()))
            shop.type = _random_shop();
    }
...
object_class_type item_in_shop(shop_type shop_type)
{
    ...
    case SHOP_GADGET:
    {
        ...
        return random_choose_weighted(
             100, OBJ_WANDS,
              50, OBJ_TALISMANS, // Evoked even if they don't use evo
              30, OBJ_MISCELLANY,
              20, OBJ_STAVES,    // Since they use evocations skill too
                                 // (and aren't covered by OBJ_WEAPONS)
              15, OBJ_ARMOUR,    // Only for orbs
               5, OBJ_BAUBLES    // Flux bauble only
        );
    }
