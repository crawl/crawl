...
targeter_exothermism::targeter_exothermism(const actor *act, int pow, int r) :
                          targeter_beam(act, r, ZAP_EXOTHERM_RAY, pow, 0, 0)
{
}

bool targeter_exothermism::set_aim(coord_def a)
{
    if (!targeter_beam::set_aim(a))
        return false;
    ...
    for (auto c : path_taken)
    {
        ...
        for (adjacent_iterator ai(c); ai; ++ai)
            if (!cell_is_solid(*ai))
                affected_cells.insert(*ai);
    }

    return true;
}

aff_type targeter_exothermism::is_affected(coord_def loc)
{
    if (affected_cells.count(loc) > 0)
        return AFF_YES;
    for (auto c : path_taken)
        if (c == loc)
            return AFF_TRACER;
