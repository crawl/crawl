...
{
    int ac = std::max(armour_class() - stab_bypass, 0);
    int gdr = gdr_perc();
    int saved = 0;
    switch (ac_rule)
    {
    ...
    case AC_PROPORTIONAL:
        ...
        saved = std::max(saved, div_rand_round(max_damage * gdr, 100));
        return std::max(damage - saved, 0);
    ...
    saved = std::max(saved, std::min(gdr * max_damage / 100, ac / 2));
    return std::max(damage - saved, 0);
}
