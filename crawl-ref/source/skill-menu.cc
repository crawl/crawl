...
#ifdef USE_TILE_LOCAL
    virtual bool on_event(wm_event ev) override;
#endif
...
UISizeReq UISkillMenu::_get_preferred_size(Direction dim, int prosp_width)
{
#ifdef USE_TILE_LOCAL
    UISizeReq ret;
    if (!dim)
...
#endif
    auto popup = make_shared<UIPopup>(skill_menu_ui);
    skm.init(flag, 80); //arbitrary initial height
