...
bool items_similar(const item_def &item1, const item_def &item2)
{
    ...
    // If a wand's max number of charges has changed (e.g. P's Supercharge),
    // don't stack it with anything.
    if (item1.base_type == OBJ_WANDS
        && wand_max_charges(item1) != wand_max_charges(item2))
    {
        return false;
    }
...
{
    const int max_charges = wand_max_charges(stack);
