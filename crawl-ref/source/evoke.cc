...
void lunge_forward(coord_def move)
{
    ...
    // don't lunge if the command is repeating
    ...
    // don't lunge if we're in fungusform and terrified to move
    if (you.is_nervous())
        return;
    // don't lunge if we're confused
    if (you.confused())
        return;
    // don't lunge if our form can't move, as in treeform
    if (you.is_stationary())
        return;
    // don't lunge if we're being constricted
    ...
    // breaks this assumption, this targeter will need to be updated.
    ...
    for (coord_def p : beam.path_taken)
    {
        ...
        // don't lunge at invis mons, but allow the tracer to keep going
            ...
            continue;
        // don't lunge if the closest mons is non-hostile
        else if (mon && (mon->friendly() || mon->neutral()))
            return;
        // okay, the first mons along the tracer is a valid target
        else if (mon && !mons_is_firewood(*mon))
        {
            valid_target = mon;
            break;
        }
    ...
    // don't lunge if the target tile is lava or lava sea
    if (feat_is_lava(grd(beam.target)))
    {
        return;
    }
    // don't lunge if the target tile is deep water or toxic bog 
    // (shallow water is okay, if it passes the traversable path check)
    // open sea is handled in the cell_is_solid() check below
    ...
    // don't lunge if the player answers no to a dangerous terrain/trap/cloud/
    // exclusion prompt; messaging for this is handled by check_moveto().
    // we check this last, so these prompts can only occur on an actual lunge.
