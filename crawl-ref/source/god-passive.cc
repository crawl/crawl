...
void ieoh_jian_heaven_tick()
{
    if (you.attribute[ATTR_HEAVEN_ON_EARTH] == 0)
        return;
    else if (you.attribute[ATTR_HEAVEN_ON_EARTH] <= 10)
...
static void _ieoh_jian_lunge(const coord_def& old_pos)
{
    ...
    {
        if (you.weapon())
            mprf("You lunge at %s, but the weight your weapon causes you to whiff.", 
...
static void _ieoh_jian_whirlwind(const coord_def& old_pos)
{
    ...
    for (auto mons : common_targets)
    {
        ...
        {
            if (you.weapon())
                mprf("You spin to attack %s, but the weight of your weapon causes you to whiff.", 
                     mons->name(DESC_THE).c_str());
            continue;
        }
        else if (number_of_attacks > 1)
        {
            if (ieoh_jian_has_momentum(IEOH_JIAN_ATTACK_WHIRLWIND))
               mprf("You spin and attack %s repeteadly with incredible momentum!", mons->name(DESC_THE).c_str());
            else
        ...
        {
            if (ieoh_jian_has_momentum(IEOH_JIAN_ATTACK_WHIRLWIND))
               mprf("You spin and attack %s with incredible momentum!", mons->name(DESC_THE).c_str());
            else
...
void ieoh_jian_wall_jump_effects(const coord_def& old_pos)
{
    ...
    for (radius_iterator ri(you.pos(), LOS_NO_TRANS); ri; ++ri)
    {
        ...
        {
            int distract_chance = div_rand_round(12 * you.experience_level,
                                                 mon->get_hit_dice());
            if (ieoh_jian_has_momentum(IEOH_JIAN_ATTACK_WALL_JUMP))
                distract_chance = div_rand_round(distract_chance * 15, 10);
            else if (distract_chance > 50)
                distract_chance = 50; // Capped if you don't have momentum.
