...
map_knowledge_type::map_knowledge_type(const map_knowledge_type& other)
{
    copy_from(other);
}

map_knowledge_type& map_knowledge_type::operator=(
                                               const map_knowledge_type& other)
{
    if (this == &other)
        return *this;
    reset_for_overwrite();
...
const item_def* map_knowledge_type::item(const map_cell& cell) const
{
...
item_def* map_knowledge_type::item(const map_cell& cell)
{
    uint16_t index = cell._item_index;
    if (index < m_items.size())
        return &m_items[index];
...
void map_knowledge_type::set_item(map_cell& cell, const item_def& ii,
                                  bool more_items)
{
    clear_item(cell);
...
void map_knowledge_type::set_detected_item(coord_def pos)
{
...
void map_knowledge_type::clear_item(map_cell& cell)
{
    uint16_t index = cell._item_index;
    if (index < m_items.size())
    {
...
monster_type map_knowledge_type::monster(const map_cell& cell) const
{
...
const monster_info* map_knowledge_type::monsterinfo(const map_cell& cell) const
{
    uint16_t index = cell._mons_index;
    if (index < m_monsters.size())
...
monster_info* map_knowledge_type::monsterinfo(const map_cell& cell)
{
    uint16_t index = cell._mons_index;
    if (index < m_monsters.size())
        return &m_monsters[index];
...
void map_knowledge_type::set_monster(map_cell& cell, const monster_info& mi)
{
    clear_monster(cell);
...
void map_knowledge_type::set_detected_monster(coord_def pos, monster_type mons)
{
    map_cell& cell = m_cells(pos);
    clear_monster(cell);
...
void map_knowledge_type::set_invisible_monster(map_cell& cell)
{
    clear_monster(cell);
    cell.flags |= MAP_INVISIBLE_MONSTER;
}

void map_knowledge_type::clear_monster(map_cell& cell)
{
    uint16_t index = cell._mons_index;
    if (index < m_monsters.size())
    {
...
cloud_type map_knowledge_type::cloud(const map_cell& cell) const
{
...
unsigned map_knowledge_type::cloud_colour(const map_cell& cell) const
{
    uint16_t index = cell._cloud_index;
    if (index < m_clouds.size())
...
const cloud_info* map_knowledge_type::cloudinfo(const map_cell& cell) const
{
    uint16_t index = cell._cloud_index;
    if (index < m_clouds.size())
...
cloud_info* map_knowledge_type::cloudinfo(const map_cell& cell)
{
    uint16_t index = cell._cloud_index;
    if (index < m_clouds.size())
        return &m_clouds[index];
...
void map_knowledge_type::set_cloud(map_cell& cell, const cloud_info& ci)
{
    clear_cloud(cell);
...
void map_knowledge_type::clear_cloud(map_cell& cell)
{
    uint16_t index = cell._cloud_index;
    if (index < m_clouds.size())
    {
...
void map_knowledge_type::clear(map_cell& cell)
{
    clear_cloud(cell);
    clear_item(cell);
    clear_monster(cell);
...
void map_knowledge_type::clear_data(map_cell& cell)
{
...
void map_knowledge_type::copy_at(coord_def pos,
                                 const map_knowledge_type& other,
                                 coord_def other_pos)
{
    map_cell& cell = m_cells(pos);
...
void map_knowledge_type::reset()
{
    reset_for_overwrite();
    m_cells.init(map_cell());
}

void map_knowledge_type::reset_for_overwrite()
{
    m_clouds.clear();
    m_items.clear();
    m_monsters.clear();
...
void map_knowledge_type::copy_from(const map_knowledge_type& other)
{
    for (int y = 0; y < GYM; ++y)
    {
        for (int x = 0; x < GXM; ++x)
...
map_feature map_knowledge_type::get_map_feature(const map_cell& cell) const
{
...
bool map_knowledge_type::update_cloud_state(map_cell& cell)
{
    if (cell.visible())
