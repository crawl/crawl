...
bool PlayerMenuEntry::get_tiles(vector<tile_def>& tileset) const
{
    ...
    if (equip_doll.parts[TILEP_PART_BOOTS] >= TILEP_BOOTS_BARDING
        && equip_doll.parts[TILEP_PART_BOOTS] <= TILEP_BOOTS_BARDING_RED)
    {
...
void SaveMenuItem::_pack_doll()
{
    ...
    if (m_save_doll.parts[TILEP_PART_BOOTS] >= TILEP_BOOTS_BARDING
        && m_save_doll.parts[TILEP_PART_BOOTS] <= TILEP_BOOTS_BARDING_RED)
    {
        flags[TILEP_PART_BOOTS] = is_naga ? TILEP_FLAG_NORMAL : TILEP_FLAG_HIDE;
    }
