#pragma once

#include "mon-book.h"

/* *************************************************************************

    This file determines which spells are contained in monster spellbooks.
    It is used by /util/gen-mst.pl to generate spellbook enums, which are
    listed in mon-mst.h.

    Template Format:

    {    mon_spellbook_type,
        {
            { spell type, frequency, flags },
            { spell type, frequency, flags },
            [...]
        }
    }

    Spellbooks are organized by shared generic books to non-uniques to
    uniques / vault monsters, base user glyph (alphabetical, lowercase
    to uppercase to numbers to misc), then loose thematic tiering.

************************************************************************* */

static const mon_spellbook mspell_list[] =
{

    /* Crimson imp, blink frog, and killer klown. */
    {  MST_BLINKER,
      {
       { SPELL_BLINK, 30, MON_SPELL_MAGICAL },
      }
    },

    /* Orc warlords and cherubs. */
    {  MST_BATTLECRY,
      {
       { SPELL_BATTLECRY, 100, MON_SPELL_VOCAL },
      }
    },

    /* Eyes of draining and ghost moths. */
    {  MST_DRAIN_GAZE,
      {
       { SPELL_DRAINING_GAZE, 100, MON_SPELL_MAGICAL | MON_SPELL_INSTANT },
      }
    },

    // Electric eels and lightning spires.
    {  MST_ZAPPER,
      {
       { SPELL_ELECTRICAL_BOLT, 100, MON_SPELL_NATURAL },
      }
    },

    // Oklob plants and acid blobs.
    {  MST_ACID_SPIT,
      {
       { SPELL_SPIT_ACID, 60, MON_SPELL_NATURAL },
      }
    },

    // War gargoyles and iron dragons.
    {  MST_SPLINTER_BREATH,
      {
       { SPELL_METAL_SPLINTERS, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH
                                     | MON_SPELL_NOISY },
      }
    },

    // Howler monkeys and demonic crawlers.
    { MST_SCREAMER,
      {
        { SPELL_WARNING_CRY, 40, MON_SPELL_VOCAL | MON_SPELL_BREATH },
      }
    },

    // Deep elf annihilators and draconian annihilators.
    {  MST_ANNIHILATOR,
      {
       { SPELL_LIGHTNING_BOLT, 10, MON_SPELL_WIZARD },
       { SPELL_POISON_ARROW, 10, MON_SPELL_WIZARD },
       { SPELL_IRON_SHOT, 10, MON_SPELL_WIZARD },
       { SPELL_LEHUDIBS_CRYSTAL_SPEAR, 10, MON_SPELL_WIZARD },
       { SPELL_BLINK, 20, MON_SPELL_WIZARD },
      }
    },

    // ('a') Ants and formicids.
    {  MST_ENTROPY_WEAVER,
      {
       { SPELL_ENTROPIC_WEAVE, 30, MON_SPELL_WIZARD },
      }
    },

    // ('b') Bats, birds, and butterflies.
    {  MST_SHARD_SHRIKE,
      {
        { SPELL_THROW_ICICLE, 20, MON_SPELL_NATURAL | MON_SPELL_BREATH },
      }
    },

    // ('c') Centaurs and such.
    {  MST_FAUN,
      {
       { SPELL_CORONA, 20, MON_SPELL_WIZARD },
       { SPELL_STRIP_WILLPOWER, 30, MON_SPELL_WIZARD },
      }
    },

    {  MST_SATYR,
      {
       { SPELL_BATTLECRY, 30, MON_SPELL_VOCAL },
       { SPELL_CAUSE_FEAR, 30, MON_SPELL_WIZARD },
       { SPELL_SLEEP, 10, MON_SPELL_WIZARD },
      }
    },

    // ('e') Elves.
    {  MST_DEEP_ELF_FIRE_MAGE,
      {
       { SPELL_STICKY_FLAME_RANGE, 10, MON_SPELL_WIZARD },
       { SPELL_BOLT_OF_FIRE, 20, MON_SPELL_WIZARD },
       { SPELL_FIRE_ELEMENTALS, 20, MON_SPELL_WIZARD },
       { SPELL_BLINK, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_DEEP_ELF_AIR_MAGE,
      {
       { SPELL_LIGHTNING_BOLT, 30, MON_SPELL_WIZARD },
       { SPELL_BLINK, 20, MON_SPELL_WIZARD },
      }
    },

    {  MST_DEEP_ELF_KNIGHT,
      {
       { SPELL_THROW_ICICLE, 30, MON_SPELL_WIZARD },
       { SPELL_FORCE_LANCE, 10, MON_SPELL_WIZARD },
       { SPELL_HASTE, 10, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_DEEP_ELF_ARCHER,
      {
       { SPELL_SLOW, 10, MON_SPELL_WIZARD },
       { SPELL_CONFUSE, 10, MON_SPELL_WIZARD },
       { SPELL_REPEL_MISSILES, 10, MON_SPELL_WIZARD },
       { SPELL_BLINK_AWAY, 40, MON_SPELL_WIZARD | MON_SPELL_SHORT_RANGE },
      }
    },

    {  MST_DEEP_ELF_ELEMENTALIST,
      {
       { SPELL_FREEZING_CLOUD, 10, MON_SPELL_WIZARD },
       { SPELL_FIREBALL, 10, MON_SPELL_WIZARD },
       { SPELL_AWAKEN_EARTH, 40, MON_SPELL_WIZARD },
       { SPELL_REPEL_MISSILES, 10, MON_SPELL_WIZARD },
       { SPELL_HASTE, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_DEEP_ELF_SORCERER,
      {
       { SPELL_CORROSIVE_BOLT, 20, MON_SPELL_WIZARD },
       { SPELL_BANISHMENT, 10, MON_SPELL_WIZARD },
       { SPELL_HASTE, 20, MON_SPELL_WIZARD },
       { SPELL_HURL_DAMNATION, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_DEEP_ELF_HIGH_PRIEST,
      {
       { SPELL_AURA_OF_BRILLIANCE, 30, MON_SPELL_PRIEST },
       { SPELL_CALL_DOWN_DAMNATION, 10, MON_SPELL_PRIEST },
       { SPELL_MALIGN_OFFERING, 20, MON_SPELL_PRIEST },
       { SPELL_SMITING, 20, MON_SPELL_PRIEST },
      }
    },

    {  MST_DEEP_ELF_DEMONOLOGIST,
      {
       { SPELL_SUMMON_DEMON, 20, MON_SPELL_WIZARD },
       { SPELL_SUMMON_GREATER_DEMON, 20, MON_SPELL_WIZARD },
       { SPELL_BANISHMENT, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_DEEP_ELF_DEATH_MAGE,
      {
       { SPELL_BOLT_OF_DRAINING, 20, MON_SPELL_WIZARD },
       { SPELL_VAMPIRIC_DRAINING, 20, MON_SPELL_WIZARD },
       { SPELL_CALL_LOST_SOUL, 10, MON_SPELL_WIZARD },
      }
    },

    // ('f') Mobile plants and fungi.
    {  MST_THORN_HUNTER,
      {
       { SPELL_THORN_VOLLEY, 40, MON_SPELL_NATURAL },
       { SPELL_WALL_OF_BRAMBLES, 20, MON_SPELL_MAGICAL },
      }
    },

    {  MST_DEATHCAP,
      {
       { SPELL_DRAIN_LIFE, 30, MON_SPELL_MAGICAL },
      }
    },

    {  MST_SHAMBLING_MANGROVE,
      {
       { SPELL_GRASPING_ROOTS, 40, MON_SPELL_NATURAL },
      }
    },

    // ('g') Small humanoids.
    {  MST_BOUDA,
      {
       { SPELL_HEAL_OTHER, 20, MON_SPELL_PRIEST },
       { SPELL_MINOR_HEALING, 20, MON_SPELL_PRIEST | MON_SPELL_EMERGENCY },
       { SPELL_WEAKENING_GAZE, 20, MON_SPELL_PRIEST | MON_SPELL_INSTANT },
       { SPELL_HUNTING_CALL, 60, MON_SPELL_VOCAL },
      }
    },

    {  MST_BOGGART,
      {
       { SPELL_SHADOW_CREATURES, 30, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD },
       { SPELL_BLINK, 10, MON_SPELL_WIZARD },
      }
    },

    // ('h') Carnivorous quadrupeds.
    {  MST_BEAR,
      {
       { SPELL_BERSERKER_RAGE, 100, MON_SPELL_NATURAL | MON_SPELL_EMERGENCY },
      }
    },

    {  MST_HELL_HOUND,
      {
       { SPELL_FIRE_BREATH, 20, MON_SPELL_NATURAL | MON_SPELL_BREATH },
      }
    },

    {  MST_RAIJU,
      {
       { SPELL_BLINKBOLT, 40, MON_SPELL_MAGICAL },
      }
    },

    {  MST_HELL_HOG,
      {
       { SPELL_FIREBALL, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH },
      }
    },

    {  MST_DOOM_HOUND,
      {
       { SPELL_DOOM_HOWL, 30, MON_SPELL_NATURAL },
      }
    },

    // ('i') Spriggans.
    {  MST_SPRIGGAN_BERSERKER,
      {
       { SPELL_BROTHERS_IN_ARMS, 20, MON_SPELL_PRIEST },
       { SPELL_BERSERKER_RAGE, 20, MON_SPELL_PRIEST },
       { SPELL_TROGS_HAND, 20, MON_SPELL_PRIEST },
      }
    },

    {  MST_SPRIGGAN_DRUID,
      {
       { SPELL_STONE_ARROW, 20, MON_SPELL_WIZARD },
       { SPELL_DRUIDS_CALL, 10, MON_SPELL_WIZARD },
       { SPELL_AWAKEN_FOREST, 20, MON_SPELL_WIZARD },
       { SPELL_MINOR_HEALING, 10, MON_SPELL_WIZARD | MON_SPELL_EMERGENCY },
      }
    },

    {  MST_SPRIGGAN_AIR_MAGE,
      {
       { SPELL_LIGHTNING_BOLT, 20, MON_SPELL_WIZARD },
       { SPELL_AIRSTRIKE, 30, MON_SPELL_WIZARD },
       { SPELL_REPEL_MISSILES, 20, MON_SPELL_WIZARD },
      }
    },

    // ('k') Drakes.
    { MST_RIME_DRAKE,
      {
        { SPELL_FLASH_FREEZE, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH },
      }
    },

    {  MST_SWAMP_DRAKE,
      {
       { SPELL_NOXIOUS_CLOUD, 40, MON_SPELL_NATURAL | MON_SPELL_BREATH },
      }
    },

    {  MST_WIND_DRAKE,
      {
       { SPELL_WIND_BLAST, 40, MON_SPELL_NATURAL | MON_SPELL_BREATH },
       { SPELL_AIRSTRIKE, 30, MON_SPELL_NATURAL },
       { SPELL_REPEL_MISSILES, 30, MON_SPELL_NATURAL },
      }
    },

    {  MST_LINDWURM,
      {
       { SPELL_FIRE_BREATH, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH },
      }
    },

    {  MST_DEATH_DRAKE,
      {
       { SPELL_MIASMA_BREATH, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH },
      }
    },

    // ('l') Lizards.
    {  MST_BASILISK,
      {
       { SPELL_PETRIFY, 60, MON_SPELL_MAGICAL },
      }
    },

    // ('m') Merfolk and friends.
    {  MST_MERFOLK_SIREN,
      {
       { SPELL_SIREN_SONG, 160, MON_SPELL_WIZARD },
      }
    },

    {  MST_WATER_NYMPH,
      {
       { SPELL_WATERSTRIKE, 60, MON_SPELL_WIZARD },
      }
    },

    {  MST_MERFOLK_AVATAR,
      {
       { SPELL_AVATAR_SONG, 160, MON_SPELL_WIZARD },
      }
    },

    {  MST_MERFOLK_AQUAMANCER,
      {
        { SPELL_PRIMAL_WAVE, 10, MON_SPELL_WIZARD },
        { SPELL_STEAM_BALL, 20, MON_SPELL_WIZARD },
        { SPELL_THROW_ICICLE, 20, MON_SPELL_WIZARD },
        { SPELL_BLINK, 10, MON_SPELL_WIZARD | MON_SPELL_EMERGENCY },
      }
    },

    // ('n') Undead corpses.
    {  MST_BOG_BODY,
      {
       { SPELL_BOLT_OF_COLD, 30, MON_SPELL_WIZARD },
       { SPELL_SLOW, 30, MON_SPELL_WIZARD },
      }
    },

    // ('o') Orcs.
    {  MST_ORC_PRIEST,
      {
       { SPELL_CANTRIP, 10, MON_SPELL_PRIEST },
       { SPELL_PAIN, 20, MON_SPELL_PRIEST },
       { SPELL_SMITING, 10, MON_SPELL_PRIEST },
       { SPELL_HEAL_OTHER, 10, MON_SPELL_PRIEST },
      }
    },

    // Shared with Sigmund.
    {  MST_ORC_WIZARD,
      {
       { SPELL_MAGIC_DART, 10, MON_SPELL_WIZARD },
       { SPELL_THROW_FLAME, 10, MON_SPELL_WIZARD },
       { SPELL_CONFUSE, 20, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_ORC_KNIGHT,
      {
       { SPELL_BATTLECRY, 40, MON_SPELL_VOCAL },
      }
    },

    {  MST_ORC_HIGH_PRIEST,
      {
       { SPELL_PAIN, 10, MON_SPELL_PRIEST },
       { SPELL_SMITING, 10, MON_SPELL_PRIEST },
       { SPELL_SUMMON_DEMON, 20, MON_SPELL_PRIEST },
       { SPELL_HEAL_OTHER, 10, MON_SPELL_PRIEST },
      }
    },

    {  MST_ORC_SORCERER,
      {
       { SPELL_BOLT_OF_FIRE, 10, MON_SPELL_WIZARD },
       { SPELL_BOLT_OF_DRAINING, 10, MON_SPELL_WIZARD },
       { SPELL_SUMMON_DEMON, 10, MON_SPELL_WIZARD },
       { SPELL_PARALYSE, 10, MON_SPELL_WIZARD },
      }
    },

    // ('p') Humans.
    {  MST_NECROMANCER,
      {
       { SPELL_BOLT_OF_DRAINING, 20, MON_SPELL_WIZARD },
       { SPELL_AGONY_RANGE, 10, MON_SPELL_WIZARD },
       { SPELL_BIND_SOULS, 30, MON_SPELL_WIZARD },
      }
    },

    // Wizard set is also used by Erolcha.
    {  MST_WIZARD,
      {
       { SPELL_FIREBALL, 10, MON_SPELL_WIZARD },
       { SPELL_BANISHMENT, 10, MON_SPELL_WIZARD },
       { SPELL_BLINK, 10, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_DEATH_KNIGHT,
      {
       { SPELL_AGONY_RANGE, 10, MON_SPELL_PRIEST },
       { SPELL_INJURY_MIRROR, 20, MON_SPELL_PRIEST },
      }
    },

    {  MST_HELL_KNIGHT,
      {
       { SPELL_BOLT_OF_FIRE, 20, MON_SPELL_PRIEST },
       { SPELL_BLINK, 10, MON_SPELL_PRIEST | MON_SPELL_EMERGENCY },
       { SPELL_HASTE, 20, MON_SPELL_PRIEST },
      }
    },

    {  MST_VAULT_SENTINEL,
      {
       { SPELL_WARNING_CRY, 50, MON_SPELL_VOCAL | MON_SPELL_BREATH },
       { SPELL_SENTINEL_MARK, 60, MON_SPELL_WIZARD },
      }
    },

    {  MST_IRONBOUND_PRESERVER,
      {
       { SPELL_INJURY_BOND, 20, MON_SPELL_WIZARD },
       { SPELL_MINOR_HEALING, 40, MON_SPELL_WIZARD },
      }
    },

    {  MST_IRONBOUND_CONVOKER,
      {
       { SPELL_WORD_OF_RECALL, 30, MON_SPELL_WIZARD },
       { SPELL_MIGHT_OTHER, 30, MON_SPELL_WIZARD },
      }
    },

    {  MST_VAULT_WARDEN,
      {
       { SPELL_SEAL_DOORS, 50, MON_SPELL_WIZARD },
      }
    },

    {  MST_IRONBOUND_FROSTHEART,
      {
       { SPELL_CREEPING_FROST, 50, MON_SPELL_WIZARD },
      }
    },

    { MST_IMPERIAL_MYRMIDON,
      {
       { SPELL_SLOW, 10, MON_SPELL_WIZARD },
       { SPELL_CONFUSE, 10, MON_SPELL_WIZARD },
       { SPELL_AGONY_RANGE, 10, MON_SPELL_WIZARD },
      }
    },

    { MST_SERVANT_OF_WHISPERS,
      {
       { SPELL_STILL_WINDS, 20, MON_SPELL_PRIEST },
       { SPELL_LIGHTNING_BOLT, 40, MON_SPELL_PRIEST },
      }
    },

    { MST_RAGGED_HIEROPHANT,
      {
       { SPELL_INJURY_BOND, 20, MON_SPELL_WIZARD },
       { SPELL_RESONANCE_STRIKE, 40, MON_SPELL_WIZARD },
      }
    },

    { MST_HALAZID_WARLOCK,
      {
       { SPELL_GHOSTLY_SACRIFICE, 20, MON_SPELL_WIZARD },
       { SPELL_BOLT_OF_DRAINING, 20, MON_SPELL_WIZARD },
       { SPELL_BOLT_OF_COLD, 20, MON_SPELL_WIZARD },
       { SPELL_SHADOW_CREATURES, 20, MON_SPELL_WIZARD },
      }
    },

    { MST_KILLER_KLOWN,
      {
       { SPELL_BLINK, 30, MON_SPELL_MAGICAL },
       { SPELL_THROW_PIE, 20, MON_SPELL_NATURAL },
      }
    },

    // ('q') Classed draconians.

    {  MST_DRACONIAN_SCORCHER,
      {
       { SPELL_BOLT_OF_FIRE, 10, MON_SPELL_WIZARD },
       { SPELL_BOLT_OF_MAGMA, 20, MON_SPELL_WIZARD },
       { SPELL_FIREBALL, 20, MON_SPELL_WIZARD },
       { SPELL_HURL_DAMNATION, 10, MON_SPELL_WIZARD },
       { SPELL_CALL_DOWN_DAMNATION, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_DRACONIAN_KNIGHT,
      {
       { SPELL_THROW_ICICLE, 20, MON_SPELL_WIZARD },
       { SPELL_BOLT_OF_COLD, 10, MON_SPELL_WIZARD },
       { SPELL_HASTE, 10, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_DRACONIAN_STORMCALLER,
      {
        { SPELL_SMITING, 20, MON_SPELL_PRIEST },
        { SPELL_SUMMON_DRAKES, 20, MON_SPELL_PRIEST },
        { SPELL_UPHEAVAL, 20, MON_SPELL_PRIEST },
      }
    },

    {  MST_DRACONIAN_SHIFTER,
      {
       { SPELL_DIMENSION_ANCHOR, 10, MON_SPELL_WIZARD },
       { SPELL_BLINK_OTHER, 20, MON_SPELL_WIZARD },
       { SPELL_BLINK_RANGE, 20, MON_SPELL_WIZARD },
       { SPELL_BLINK_ALLIES_ENCIRCLE, 10, MON_SPELL_WIZARD },
       { SPELL_BLINK_AWAY, 10, MON_SPELL_WIZARD | MON_SPELL_EMERGENCY },
      }
    },

    // ('r') rodents
    {  MST_CRYSTAL_ECHIDNA,
      {
       { SPELL_CRYSTAL_BOLT, 30, MON_SPELL_NATURAL },
       { SPELL_THROW_BARBS, 30, MON_SPELL_NATURAL },
      }
    },

    // ('s') Spiders and insects.
    {  MST_JUMPING_SPIDER,
      {
       { SPELL_BLINK_CLOSE, 30, MON_SPELL_NATURAL },
       { SPELL_BLINK_AWAY, 30, MON_SPELL_NATURAL },
      }
    },

    {  MST_ORB_SPIDER,
      {
       { SPELL_IOOD, 80, MON_SPELL_MAGICAL },
      }
    },

    {  MST_BROODMOTHER,
      {
       { SPELL_SUMMON_SPIDERS, 30, MON_SPELL_NATURAL },
      }
    },

    {  MST_CULICIVORA,
      {
       { SPELL_HEAL_OTHER, 40, MON_SPELL_NATURAL },
      }
    },

    // ('t') Testudines.
    {  MST_ALLIGATOR,
      {
       { SPELL_SPRINT, 30, MON_SPELL_NATURAL },
      }
    },

    {  MST_FIRE_CRAB,
      {
       { SPELL_FLAMING_CLOUD, 60, MON_SPELL_NATURAL },
      }
    },

    {  MST_GHOST_CRAB,
      {
       { SPELL_SPECTRAL_CLOUD, 60, MON_SPELL_NATURAL },
      }
    },

    {  MST_APOCALYPSE_CRAB,
      {
       { SPELL_CHAOS_BREATH, 60, MON_SPELL_NATURAL },
      }
    },

    // ('v') Vortices.
    {  MST_WILL_O_THE_WISP,
      {
       { SPELL_MARSHLIGHT, 40, MON_SPELL_MAGICAL },
       { SPELL_BLINK, 10, MON_SPELL_MAGICAL },
      }
    },

    // ('w') Slugs and worms.
    { MST_DART_SLUG,
      {
        { SPELL_SLUG_DART, 60, MON_SPELL_NATURAL },
      }
    },

    { MST_SWAMP_WORM,
      {
        { SPELL_HARPOON_SHOT, 80, MON_SPELL_NATURAL },
      }
    },

    // ('x') Lesser abominations and tentacled things.
    {  MST_WORLDBINDER,
      {
       { SPELL_FORCEFUL_INVITATION, 60, MON_SPELL_MAGICAL },
      }
    },

    { MST_BUNYIP,
      {
        { SPELL_WARNING_CRY, 20, MON_SPELL_VOCAL | MON_SPELL_BREATH },
      }
    },

    // ('y') Flying insects.
    {  MST_MELIAI,
      {
       { SPELL_SMITING, 20, MON_SPELL_PRIEST },
       { SPELL_HEAL_OTHER, 20, MON_SPELL_PRIEST },
      }
    },

    {  MST_QUEEN_BEE,
      {
       { SPELL_BERSERK_OTHER, 40, MON_SPELL_NATURAL },
       { SPELL_BERSERK_OTHER, 80, MON_SPELL_NATURAL | MON_SPELL_EMERGENCY },
      }
    },

    {  MST_SPARK_WASP,
      {
       { SPELL_BLINKBOLT, 40, MON_SPELL_MAGICAL | MON_SPELL_LONG_RANGE },
      }
    },

    {  MST_MOTH_OF_WRATH,
      {
       { SPELL_BERSERK_OTHER, 60, MON_SPELL_NATURAL },
      }
    },

    {  MST_SUN_MOTH,
      {
       { SPELL_STICKY_FLAME_RANGE, 20, MON_SPELL_NATURAL },
       { SPELL_ENERGY_BOLT, 20, MON_SPELL_NATURAL },
      }
    },

    // ('z') Corporeal semi-humanoid undead.
    {  MST_ANCIENT_CHAMPION,
      {
       { SPELL_IRON_SHOT, 20, MON_SPELL_WIZARD },
       { SPELL_HASTE, 20, MON_SPELL_WIZARD },
      }
    },

    {  MST_CURSE_SKULL,
      {
       { SPELL_SUMMON_UNDEAD, 40, MON_SPELL_MAGICAL | MON_SPELL_NOISY },
       { SPELL_SYMBOL_OF_TORMENT, 20, MON_SPELL_MAGICAL | MON_SPELL_NOISY },
      }
    },

    {  MST_CURSE_TOE,
      {
       { SPELL_SYMBOL_OF_TORMENT, 20, MON_SPELL_MAGICAL },
       { SPELL_SUMMON_MUSHROOMS, 40, MON_SPELL_MAGICAL },
      }
    },

    // ('A') Angels.
    {  MST_OPHAN,
      {
       { SPELL_BOLT_OF_FIRE, 30, MON_SPELL_MAGICAL },
       { SPELL_HOLY_FLAMES, 30, MON_SPELL_MAGICAL },
      }
    },

    {  MST_ANGEL,
      {
       { SPELL_MINOR_HEALING, 60, MON_SPELL_MAGICAL },
      }
    },

    {  MST_DAEVA,
      {
       { SPELL_SMITING, 60, MON_SPELL_MAGICAL },
      }
    },

    {  MST_SERAPH,
      {
       { SPELL_WARNING_CRY, 40, MON_SPELL_VOCAL| MON_SPELL_BREATH },
       { SPELL_SUMMON_HOLIES, 60, MON_SPELL_MAGICAL },
       { SPELL_INJURY_BOND, 40, MON_SPELL_MAGICAL },
       { SPELL_CLEANSING_FLAME, 30, MON_SPELL_MAGICAL },
      }
    },

    // ('B') Beetles and other insects
    {  MST_BOULDER_BEETLE,
      {
       { SPELL_ROLL, 40, MON_SPELL_NATURAL},
       { SPELL_BLINK_AWAY, 20, MON_SPELL_NATURAL},
      }
    },

    {  MST_RADROACH,
      {
       { SPELL_IRRADIATE, 40, MON_SPELL_NATURAL},
      }
    },

    // ('C') Giants.
    {  MST_FIRE_GIANT,
      {
       { SPELL_BOLT_OF_FIRE, 20, MON_SPELL_WIZARD },
       { SPELL_FIREBALL, 20, MON_SPELL_WIZARD },
      }
    },

    {  MST_FROST_GIANT,
      {
       { SPELL_BOLT_OF_COLD, 60, MON_SPELL_WIZARD },
      }
    },

    {  MST_TITAN,
      {
       { SPELL_LIGHTNING_BOLT, 20, MON_SPELL_WIZARD },
       { SPELL_MINOR_HEALING, 30, MON_SPELL_WIZARD },
       { SPELL_AIRSTRIKE, 20, MON_SPELL_WIZARD },
      }
    },

    {  MST_IRON_GIANT,
      {
       { SPELL_IRON_SHOT, 20, MON_SPELL_WIZARD },
       { SPELL_HARPOON_SHOT, 40, MON_SPELL_WIZARD },
       { SPELL_THROW_ALLY, 40, MON_SPELL_NATURAL },
      }
    },

    {  MST_TAINTED_LEVIATHAN,
      {
       { SPELL_MESMERISE, 30, MON_SPELL_NATURAL },
      }
    },

    // ('D') Dragons.
    {  MST_STEAM_DRAGON,
      {
       { SPELL_STEAM_BALL, 40, MON_SPELL_NATURAL | MON_SPELL_BREATH
                                | MON_SPELL_NOISY },
      }
    },

    {  MST_ACID_DRAGON,
      {
       { SPELL_SPIT_ACID, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH
                                        | MON_SPELL_NOISY },
      }
    },

    {  MST_SWAMP_DRAGON,
      {
       { SPELL_POISONOUS_CLOUD, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH
                                     | MON_SPELL_NOISY },
      }
    },

    {  MST_FIRE_DRAGON_BREATH,
      {
       { SPELL_FIRE_BREATH, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH
                                 | MON_SPELL_NOISY },
      }
    },

    {  MST_ICE_DRAGON_BREATH,
      {
       { SPELL_COLD_BREATH, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH
                                 | MON_SPELL_NOISY },
      }
    },

    {  MST_STORM_DRAGON,
      {
       { SPELL_LIGHTNING_BOLT, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH
                                    | MON_SPELL_NOISY },
      }
    },

    {  MST_SHADOW_DRAGON,
      {
       { SPELL_BOLT_OF_DRAINING, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH },
      }
    },

    {  MST_QUICKSILVER_DRAGON,
      {
       { SPELL_QUICKSILVER_BOLT, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH
                                      | MON_SPELL_NOISY },
      }
    },

    {  MST_GOLDEN_DRAGON,
      {
       { SPELL_BOLT_OF_FIRE, 20, MON_SPELL_NATURAL | MON_SPELL_BREATH
                                  | MON_SPELL_NOISY },
       { SPELL_BOLT_OF_COLD, 20, MON_SPELL_NATURAL | MON_SPELL_BREATH
                                  | MON_SPELL_NOISY },
       { SPELL_POISONOUS_CLOUD, 20, MON_SPELL_NATURAL | MON_SPELL_BREATH
                                  | MON_SPELL_NOISY },
      }
    },

    {  MST_PEARL_DRAGON,
      {
       { SPELL_HOLY_BREATH, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH
                                 | MON_SPELL_NOISY },
      }
    },

    // ('E') Elementals.
    {  MST_AIR_ELEMENTAL,
      {
       { SPELL_STUNNING_BURST, 40, MON_SPELL_NATURAL},
      }
    },

    {  MST_ELEMENTAL_WELLSPRING,
      {
       { SPELL_PRIMAL_WAVE, 60, MON_SPELL_MAGICAL },
       { SPELL_WATER_ELEMENTALS, 60, MON_SPELL_MAGICAL },
      }
    },

    {  MST_IRON_ELEMENTAL,
      {
       { SPELL_IRON_SHOT, 20, MON_SPELL_MAGICAL },
       { SPELL_METAL_SPLINTERS, 20, MON_SPELL_MAGICAL },
       { SPELL_SLOW, 20, MON_SPELL_MAGICAL },
      }
    },

    // ('G') Organs.
    {  MST_FLOATING_EYE,
      {
       { SPELL_PARALYSIS_GAZE, 80, MON_SPELL_MAGICAL | MON_SPELL_INSTANT },
      }
    },

    {  MST_GOLDEN_EYE,
      {
       { SPELL_CONFUSION_GAZE, 100, MON_SPELL_MAGICAL | MON_SPELL_INSTANT },
       { SPELL_BLINK, 30, MON_SPELL_MAGICAL },
      }
    },

    {  MST_EYE_OF_DEVASTATION,
      {
       { SPELL_ENERGY_BOLT, 60, MON_SPELL_MAGICAL },
      }
    },

    {  MST_SHINING_EYE,
      {
       { SPELL_MALMUTATE, 60, MON_SPELL_MAGICAL },
      }
    },

    {  MST_GREAT_ORB_OF_EYES,
      {
       { SPELL_PARALYSE, 20, MON_SPELL_MAGICAL },
       { SPELL_MINDBURST, 10, MON_SPELL_MAGICAL },
       { SPELL_DRAIN_MAGIC, 10, MON_SPELL_MAGICAL },
       { SPELL_CONFUSE, 10, MON_SPELL_MAGICAL },
      }
    },

    {  MST_GLOWING_ORANGE_BRAIN,
      {
       { SPELL_BRAIN_FEED, 10, MON_SPELL_MAGICAL },
       { SPELL_CAUSE_FEAR, 10, MON_SPELL_MAGICAL },
       { SPELL_SHADOW_CREATURES, 10, MON_SPELL_MAGICAL },
       { SPELL_MASS_CONFUSION, 10, MON_SPELL_MAGICAL },
       { SPELL_BLINK, 10, MON_SPELL_MAGICAL },
      }
    },

    // ('H') Animal hybrids.
    {  MST_MANTICORE,
      {
       { SPELL_THROW_BARBS, 100, MON_SPELL_NATURAL },
      }
    },

    { MST_FENSTRIDER_WITCH,
      {
       { SPELL_PARALYSE, 10, MON_SPELL_WIZARD },
       { SPELL_AGONY, 30, MON_SPELL_WIZARD },
       { SPELL_HURL_SLUDGE, 20, MON_SPELL_WIZARD },
      }
    },

    {  MST_SPHINX,
      {
       { SPELL_CONFUSE, 10, MON_SPELL_WIZARD },
       { SPELL_SLOW, 10, MON_SPELL_WIZARD },
       { SPELL_PARALYSE, 10, MON_SPELL_WIZARD },
       { SPELL_SMITING, 10, MON_SPELL_WIZARD },
      }
    },

    // ('H') Hybrids.
    {  MST_JOROGUMO,
      {
       { SPELL_BOLT_OF_DRAINING, 20, MON_SPELL_WIZARD },
       { SPELL_ENSNARE, 40, MON_SPELL_WIZARD },
      }
    },

    // ('K') Kobolds.
    {  MST_KOBOLD_DEMONOLOGIST,
      {
       { SPELL_CANTRIP, 20, MON_SPELL_WIZARD },
       { SPELL_SUMMON_MINOR_DEMON, 20, MON_SPELL_WIZARD },
       { SPELL_SUMMON_DEMON, 20, MON_SPELL_WIZARD },
      }
    },

    // ('L') Liches and the like.
    {  MST_REVENANT,
      {
       { SPELL_SPECTRAL_CLOUD, 20, MON_SPELL_WIZARD },
       { SPELL_GHOSTLY_FIREBALL, 20, MON_SPELL_WIZARD },
       { SPELL_DISPEL_UNDEAD_RANGE, 20, MON_SPELL_WIZARD },
       { SPELL_BLINK_AWAY, 10, MON_SPELL_WIZARD | MON_SPELL_EMERGENCY },
      }
    },

    {  MST_LICH,
      {
       { SPELL_BOLT_OF_COLD, 20, MON_SPELL_WIZARD },
       { SPELL_IOOD, 10, MON_SPELL_WIZARD },
       { SPELL_HASTE, 10, MON_SPELL_WIZARD },
       { SPELL_SLOW, 10, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_ANCIENT_LICH,
      {
       { SPELL_CORROSIVE_BOLT, 10, MON_SPELL_WIZARD },
       { SPELL_LEHUDIBS_CRYSTAL_SPEAR, 20, MON_SPELL_WIZARD },
       { SPELL_HASTE, 10, MON_SPELL_WIZARD },
       { SPELL_PETRIFY, 10, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_DREAD_LICH,
      {
       { SPELL_SUMMON_GREATER_DEMON, 20, MON_SPELL_WIZARD },
       { SPELL_PARALYSE, 20, MON_SPELL_WIZARD },
       { SPELL_HASTE, 10, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_STOKER,
      {
       {
        { SPELL_WIND_BLAST, 20, MON_SPELL_WIZARD },
        { SPELL_STOKE_FLAMES, 10, MON_SPELL_WIZARD },
       }
      }
    },

    // ('M') Mummies.
    {  MST_MUMMY_PRIEST,
      {
       { SPELL_SUMMON_DEMON, 10, MON_SPELL_PRIEST },
       { SPELL_SUMMON_UNDEAD, 20, MON_SPELL_PRIEST },
       { SPELL_SMITING, 10, MON_SPELL_PRIEST },
       { SPELL_SYMBOL_OF_TORMENT, 10, MON_SPELL_PRIEST },
      }
    },

    {  MST_ROYAL_MUMMY,
      {
       { SPELL_SMITING, 10, MON_SPELL_WIZARD },
       { SPELL_SYMBOL_OF_TORMENT, 10, MON_SPELL_WIZARD },
       { SPELL_SUMMON_EMPEROR_SCORPIONS, 10, MON_SPELL_WIZARD },
       { SPELL_SUMMON_SCARABS, 30, MON_SPELL_WIZARD },
      }
    },

    // ('N') Naga and other snake people.
    {  MST_NAGA,
      {
       { SPELL_SPIT_POISON, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH },
      }
    },

    {  MST_NAGA_WARRIOR,
      {
       { SPELL_SPIT_POISON, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH },
       { SPELL_BATTLECRY, 20, MON_SPELL_VOCAL },
      }
    },

    {  MST_NAGA_MAGE,
      {
       { SPELL_SPIT_POISON, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH },
       { SPELL_VENOM_BOLT, 20, MON_SPELL_WIZARD },
       { SPELL_CONCENTRATE_VENOM, 20, MON_SPELL_WIZARD },
       { SPELL_HASTE, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_NAGARAJA,
      {
       { SPELL_SPIT_POISON, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH },
       { SPELL_VENOM_BOLT, 10, MON_SPELL_WIZARD },
       { SPELL_POISON_ARROW, 20, MON_SPELL_WIZARD },
       { SPELL_DIMENSION_ANCHOR, 10, MON_SPELL_WIZARD },
       { SPELL_HASTE, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_NAGA_SHARPSHOOTER,
      {
       { SPELL_SPIT_POISON, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH },
       { SPELL_PORTAL_PROJECTILE, 60, MON_SPELL_WIZARD },
      }
    },

    {  MST_NAGA_RITUALIST,
      {
       { SPELL_SPIT_POISON, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH },
       { SPELL_OLGREBS_TOXIC_RADIANCE, 30, MON_SPELL_PRIEST },
       { SPELL_VIRULENCE, 30, MON_SPELL_PRIEST },
      }
    },

    {  MST_SALAMANDER_MYSTIC,
      {
       { SPELL_FORCE_LANCE, 10, MON_SPELL_WIZARD },
       { SPELL_BOLT_OF_MAGMA, 10, MON_SPELL_WIZARD },
       { SPELL_HASTE_OTHER, 20, MON_SPELL_WIZARD },
      }
    },

    {  MST_SALAMANDER_TYRANT,
      {
       { SPELL_ERUPTION, 20, MON_SPELL_WIZARD },
       { SPELL_PYROCLASTIC_SURGE, 60, MON_SPELL_WIZARD },
       { SPELL_WEAKENING_GAZE, 40, MON_SPELL_WIZARD | MON_SPELL_INSTANT },
      }
    },

    // ('O') Ogres.
    {  MST_OGRE_MAGE,
      {
       { SPELL_BOLT_OF_MAGMA, 20, MON_SPELL_WIZARD },
       { SPELL_HASTE_OTHER, 10, MON_SPELL_WIZARD },
       { SPELL_PARALYSE, 10, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_IRONBOUND_THUNDERHULK,
      {
       { SPELL_CALL_DOWN_LIGHTNING, 60, MON_SPELL_MAGICAL },
       { SPELL_BLINK_RANGE, 20, MON_SPELL_MAGICAL },
      }
    },

    // ('P') Immobile plants.
    {  MST_BURNING_BUSH,
      {
       { SPELL_THROW_FLAME, 60, MON_SPELL_NATURAL },
      }
    },

    {  MST_OKLOB_SAPLING,
      {
       { SPELL_SPIT_ACID, 60, MON_SPELL_NATURAL },
      }
    },

    {  MST_BALLISTOMYCETE,
      {
       { SPELL_SPORULATE, 60, MON_SPELL_NATURAL },
      }
    },

    { MST_STARFLOWER,
      {
       { SPELL_HARPOON_SHOT, 80, MON_SPELL_NATURAL },
      }
    },

    // ('Q') Tengu.
    {  MST_TENGU_CONJURER,
      {
       { SPELL_LIGHTNING_BOLT, 20, MON_SPELL_WIZARD },
       { SPELL_BATTLESPHERE, 20, MON_SPELL_WIZARD },
       { SPELL_BLINK, 10, MON_SPELL_WIZARD | MON_SPELL_EMERGENCY },
      }
    },

    {  MST_TENGU_REAVER,
      {
        { SPELL_VENOM_BOLT, 10, MON_SPELL_WIZARD },
        { SPELL_BOLT_OF_FIRE, 10, MON_SPELL_WIZARD },
        { SPELL_BOLT_OF_COLD, 10, MON_SPELL_WIZARD },
        { SPELL_CORROSIVE_BOLT, 10, MON_SPELL_WIZARD },
        { SPELL_LIGHTNING_BOLT, 10, MON_SPELL_WIZARD },
        { SPELL_BOLT_OF_DRAINING, 10, MON_SPELL_WIZARD },
      }
    },

    // ('R') Spirituals.
    {  MST_RAKSHASA,
      {
       { SPELL_FORCE_LANCE, 20, MON_SPELL_MAGICAL },
       { SPELL_PHANTOM_MIRROR, 20, MON_SPELL_MAGICAL },
       { SPELL_BLINK, 10, MON_SPELL_MAGICAL },
      }
    },

    {  MST_EFREET,
      {
       { SPELL_BOLT_OF_FIRE, 30, MON_SPELL_MAGICAL },
       { SPELL_FIREBALL, 30, MON_SPELL_MAGICAL },
      }
    },

    {  MST_DRYAD,
      {
       { SPELL_AWAKEN_VINES, 20, MON_SPELL_WIZARD },
       { SPELL_AWAKEN_FOREST, 30, MON_SPELL_WIZARD },
       { SPELL_MINOR_HEALING, 10, MON_SPELL_WIZARD | MON_SPELL_EMERGENCY },
      }
    },

    {  MST_ELEIONOMA,
      {
       { SPELL_SPLINTERSPRAY, 40, MON_SPELL_MAGICAL },
       { SPELL_WOODWEAL, 160, MON_SPELL_MAGICAL | MON_SPELL_EMERGENCY },
      }
    },

    {  MST_WENDIGO,
      {
       { SPELL_SERACFALL, 60, MON_SPELL_MAGICAL },
       { SPELL_STUNNING_BURST, 30, MON_SPELL_MAGICAL },
      }
    },

    // ('S') Snakes.
    {  MST_LAVA_SNAKE,
      {
       { SPELL_SPIT_LAVA, 100, MON_SPELL_NATURAL | MON_SPELL_BREATH },
      }
    },

    {  MST_GUARDIAN_SERPENT,
      {
       { SPELL_VENOM_BOLT, 20, MON_SPELL_WIZARD },
       { SPELL_SLOW, 20, MON_SPELL_WIZARD },
       { SPELL_BLINK_ALLIES_ENCIRCLE, 20, MON_SPELL_WIZARD },
      }
    },

    {  MST_SHOCK_SERPENT,
      {
       { SPELL_ELECTRICAL_BOLT, 30, MON_SPELL_NATURAL },
      }
    },

    // ('T') Trolls.
    {  MST_DEEP_TROLL_EARTH_MAGE,
      {
       { SPELL_LRD, 30, MON_SPELL_WIZARD },
       { SPELL_DIG, 30, MON_SPELL_WIZARD },
      }
    },

    {  MST_DEEP_TROLL_SHAMAN,
      {
       { SPELL_HASTE_OTHER, 20, MON_SPELL_PRIEST },
       { SPELL_MIGHT_OTHER, 20, MON_SPELL_PRIEST },
      }
    },

    // ('V') Vampires.
    {  MST_VAMPIRE,
      {
       { SPELL_VAMPIRIC_DRAINING, 40, MON_SPELL_WIZARD },
       { SPELL_CONFUSE, 20, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_VAMPIRE_MAGE,
      {
       { SPELL_BOLT_OF_DRAINING, 10, MON_SPELL_WIZARD },
       { SPELL_SUMMON_UNDEAD, 10, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 20, MON_SPELL_WIZARD },
       { SPELL_VAMPIRIC_DRAINING, 40, MON_SPELL_WIZARD },
      }
    },

    {  MST_VAMPIRE_KNIGHT,
      {
       { SPELL_BLINK_CLOSE, 20, MON_SPELL_WIZARD },
       { SPELL_PARALYSE, 10, MON_SPELL_WIZARD },
       { SPELL_HASTE, 10, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD },
       { SPELL_VAMPIRIC_DRAINING, 40, MON_SPELL_WIZARD },
      }
    },

    // ('W') Incorporeal undead.
    {  MST_SHADOW,
      {
       { SPELL_INVISIBILITY, 30, MON_SPELL_NATURAL },
      }
    },

    {  MST_PHANTASMAL_WARRIOR,
      {
       { SPELL_BLINK_CLOSE, 60, MON_SPELL_MAGICAL },
      }
    },

    {  MST_EIDOLON,
      {
       { SPELL_BOLT_OF_DRAINING, 20, MON_SPELL_MAGICAL },
       { SPELL_CAUSE_FEAR, 20, MON_SPELL_MAGICAL },
      }
    },

    {  MST_FLAYED_GHOST,
      {
       { SPELL_FLAY, 40, MON_SPELL_MAGICAL },
      }
    },

    {  MST_PUTRID_MOUTH,
      {
       { SPELL_MIASMA_BREATH, 40, MON_SPELL_NATURAL | MON_SPELL_BREATH },
       { SPELL_WARNING_CRY, 20, MON_SPELL_VOCAL | MON_SPELL_BREATH },
      }
    },

    // ('X') Greater horrors and tentacled things.
    {  MST_KRAKEN,
      {
       { SPELL_CREATE_TENTACLES, 40, MON_SPELL_NATURAL },
       { SPELL_INK_CLOUD, 20, MON_SPELL_NATURAL | MON_SPELL_EMERGENCY },
      }
    },

    {  MST_THRASHING_HORROR,
      {
       { SPELL_MIGHT, 60, MON_SPELL_NATURAL | MON_SPELL_EMERGENCY },
      }
    },

    {  MST_TENTACLED_STARSPAWN,
      {
       { SPELL_CREATE_TENTACLES, 60, MON_SPELL_NATURAL },
      }
    },

    {  MST_NAMELESS,
      {
       { SPELL_ABJURATION, 60, MON_SPELL_NATURAL },
      }
    },

    {  MST_MLIOGLOTL,
      {
       { SPELL_MIGHT, 20, MON_SPELL_NATURAL },
       { SPELL_CAUSE_FEAR, 20, MON_SPELL_PRIEST },
       { SPELL_CORRUPT_LOCALE, 30, MON_SPELL_PRIEST },
      }
    },

    // ('Y') Bovids and elephants.
    {  MST_CATOBLEPAS,
      {
       { SPELL_PETRIFYING_CLOUD, 40, MON_SPELL_NATURAL },
      }
    },

    { MST_DREAM_SHEEP,
      {
       { SPELL_DREAM_DUST, 40, MON_SPELL_NATURAL },
      }
    },

    {  MST_HELLEPHANT,
      {
       { SPELL_FIRE_BREATH, 40, MON_SPELL_NATURAL | MON_SPELL_BREATH },
       { SPELL_BLINK, 30, MON_SPELL_MAGICAL },
      }
    },

    // ('5') Minor demons.
    {  MST_WHITE_IMP,
      {
       { SPELL_THROW_FROST, 40, MON_SPELL_MAGICAL },
      }
    },

    {  MST_SHADOW_IMP,
      {
       { SPELL_PAIN, 20, MON_SPELL_MAGICAL },
      }
    },

    // ('3') Common demons.
    {  MST_YNOXINUL,
      {
       { SPELL_IRON_SHOT, 20, MON_SPELL_MAGICAL },
       { SPELL_SUMMON_UFETUBUS, 40, MON_SPELL_MAGICAL },
      }
    },

    {  MST_SMOKE_DEMON,
      {
       { SPELL_STICKY_FLAME_RANGE, 20, MON_SPELL_MAGICAL },
       { SPELL_STEAM_BALL, 20, MON_SPELL_MAGICAL },
       { SPELL_SMITING, 20, MON_SPELL_MAGICAL },
      }
    },

    {  MST_SOUL_EATER,
      {
       { SPELL_DRAIN_LIFE, 30, MON_SPELL_MAGICAL },
      }
    },

    {  MST_NEQOXEC,
      {
       { SPELL_MALMUTATE, 20, MON_SPELL_MAGICAL },
       { SPELL_BRAIN_FEED, 20, MON_SPELL_MAGICAL },
      }
    },

    // ('2') Greater demons.
    {  MST_SHADOW_DEMON,
      {
       { SPELL_SHADOW_CREATURES, 30, MON_SPELL_MAGICAL },
      }
    },

    {  MST_GREEN_DEATH,
      {
       { SPELL_POISON_ARROW, 20, MON_SPELL_MAGICAL },
       { SPELL_POISONOUS_CLOUD, 20, MON_SPELL_MAGICAL },
       { SPELL_VENOM_BOLT, 20, MON_SPELL_MAGICAL },
      }
    },

    {  MST_BALRUG,
      {
       { SPELL_STICKY_FLAME_RANGE, 10, MON_SPELL_MAGICAL },
       { SPELL_BOLT_OF_FIRE, 10, MON_SPELL_MAGICAL },
       { SPELL_FIREBALL, 20, MON_SPELL_MAGICAL },
       { SPELL_SMITING, 20, MON_SPELL_MAGICAL },
      }
    },

    {  MST_BLIZZARD_DEMON,
      {
       { SPELL_BOLT_OF_COLD, 20, MON_SPELL_MAGICAL },
       { SPELL_LIGHTNING_BOLT, 20, MON_SPELL_MAGICAL },
       { SPELL_FREEZING_CLOUD, 10, MON_SPELL_MAGICAL },
       { SPELL_AIRSTRIKE, 10, MON_SPELL_MAGICAL },
      }
    },

    {  MST_CACODEMON,
      {
       { SPELL_ENERGY_BOLT, 10, MON_SPELL_MAGICAL },
       { SPELL_SLOW, 10, MON_SPELL_MAGICAL },
       { SPELL_CONFUSE, 10, MON_SPELL_MAGICAL },
       { SPELL_MALMUTATE, 10, MON_SPELL_MAGICAL },
       { SPELL_DIG, 20, MON_SPELL_MAGICAL },
      }
    },

    {  MST_HELLION,
      {
       { SPELL_CALL_DOWN_DAMNATION, 60, MON_SPELL_MAGICAL },
      }
    },

    {  MST_TORMENTOR,
      {
       { SPELL_SYMBOL_OF_TORMENT, 30, MON_SPELL_MAGICAL },
      }
    },

    // ('1') Fiends and friends.
    {  MST_EXECUTIONER,
      {
       { SPELL_HASTE, 40, MON_SPELL_MAGICAL },
      }
    },

    {  MST_BRIMSTONE_FIEND,
      {
       { SPELL_HURL_DAMNATION, 30, MON_SPELL_MAGICAL },
       { SPELL_SYMBOL_OF_TORMENT, 20, MON_SPELL_MAGICAL },
      }
    },

    {  MST_ICE_FIEND,
      {
       { SPELL_BOLT_OF_COLD, 40, MON_SPELL_MAGICAL },
       { SPELL_SYMBOL_OF_TORMENT, 20, MON_SPELL_MAGICAL },
      }
    },

    {  MST_TZITZIMITL,
      {
       { SPELL_BOLT_OF_DRAINING, 30, MON_SPELL_MAGICAL },
       { SPELL_SYMBOL_OF_TORMENT, 20, MON_SPELL_MAGICAL },
       { SPELL_DISPEL_UNDEAD_RANGE, 20, MON_SPELL_MAGICAL },
      }
    },

    {  MST_HELL_SENTINEL,
      {
       { SPELL_HURL_DAMNATION, 30, MON_SPELL_MAGICAL },
       { SPELL_IRON_SHOT, 10, MON_SPELL_MAGICAL },
      }
    },

    // ('6') Demonspawn.
    {  MST_DEMONSPAWN_WARMONGER,
      {
       { SPELL_SAP_MAGIC, 20, MON_SPELL_PRIEST },
       { SPELL_HASTE_OTHER, 30, MON_SPELL_PRIEST },
      }
    },

    {  MST_DEMONSPAWN_BLOOD_SAINT,
      {
       { SPELL_LEGENDARY_DESTRUCTION, 40, MON_SPELL_PRIEST },
       { SPELL_CALL_OF_CHAOS, 20, MON_SPELL_PRIEST },
      }
    },

    {  MST_DEMONSPAWN_CORRUPTER,
      {
       { SPELL_PLANEREND, 40, MON_SPELL_PRIEST },
       { SPELL_ENTROPIC_WEAVE, 20, MON_SPELL_PRIEST },
      }
    },

    {  MST_DEMONSPAWN_BLACK_SUN,
      {
       { SPELL_BOLT_OF_DRAINING, 20, MON_SPELL_PRIEST },
       { SPELL_DISPEL_UNDEAD_RANGE, 20, MON_SPELL_PRIEST },
       { SPELL_BLACK_MARK, 30, MON_SPELL_PRIEST },
      }
    },

    // ('8') Animate / statues.
    {  MST_USHABTI,
      {
       { SPELL_DEATH_RATTLE, 30, MON_SPELL_MAGICAL },
       { SPELL_WARNING_CRY, 60, MON_SPELL_VOCAL | MON_SPELL_BREATH },
       { SPELL_DISPEL_UNDEAD_RANGE, 40, MON_SPELL_MAGICAL },
      }
    },

    {  MST_PEACEKEEPER,
      {
       { SPELL_THROW_BARBS, 30, MON_SPELL_NATURAL },
       { SPELL_BATTLECRY, 30, MON_SPELL_VOCAL },
      }
    },

    {  MST_CRYSTAL_GUARDIAN,
      {
       { SPELL_CRYSTAL_BOLT, 60, MON_SPELL_MAGICAL },
      }
    },

    {  MST_ELECTRIC_GOLEM,
      {
       { SPELL_LIGHTNING_BOLT, 40, MON_SPELL_MAGICAL },
       { SPELL_BLINK, 20, MON_SPELL_MAGICAL },
      }
    },

    {  MST_ICE_STATUE,
      {
       { SPELL_BOLT_OF_COLD, 20, MON_SPELL_MAGICAL },
       { SPELL_THROW_ICICLE, 20, MON_SPELL_MAGICAL },
       { SPELL_FREEZING_CLOUD, 20, MON_SPELL_MAGICAL },
       { SPELL_SUMMON_ICE_BEAST, 40, MON_SPELL_MAGICAL },
      }
    },

    {  MST_OBSIDIAN_STATUE,
      {
       { SPELL_SUMMON_MINOR_DEMON, 30, MON_SPELL_MAGICAL },
       { SPELL_SUMMON_DEMON, 30, MON_SPELL_MAGICAL },
       { SPELL_MESMERISE, 30, MON_SPELL_MAGICAL },
      }
    },

    {  MST_ORANGE_CRYSTAL_STATUE,
      {
       { SPELL_SHADOW_CREATURES, 40, MON_SPELL_MAGICAL },
       { SPELL_BRAIN_FEED, 20, MON_SPELL_MAGICAL },
       { SPELL_DRAINING_GAZE, 20, MON_SPELL_MAGICAL },
       { SPELL_MASS_CONFUSION, 20, MON_SPELL_MAGICAL },
      }
    },

    {  MST_STRANGE_MACHINE,
      {
       { SPELL_SUMMON_ILLUSION, 40, MON_SPELL_MAGICAL },
       { SPELL_SHADOW_CREATURES, 20, MON_SPELL_MAGICAL },
      }
    },

    // ('9') Gargoyles.
    {  MST_GARGOYLE,
      {
       { SPELL_STONE_ARROW, 60, MON_SPELL_NATURAL },
      }
    },

    {  MST_MOLTEN_GARGOYLE,
      {
       { SPELL_BOLT_OF_MAGMA, 60, MON_SPELL_NATURAL },
      }
    },

    {  MST_NARGUN,
      {
       { SPELL_PETRIFY, 60, MON_SPELL_NATURAL },
      }
    },

    // ('*') Concentrated orbs.
    {  MST_WRETCHED_STAR,
      {
       { SPELL_FORCE_LANCE, 60, MON_SPELL_MAGICAL },
       { SPELL_CORRUPTING_PULSE, 40, MON_SPELL_MAGICAL },
      }
    },

    {  MST_ORB_OF_FIRE,
      {
       { SPELL_BOLT_OF_FIRE, 30, MON_SPELL_MAGICAL },
       { SPELL_FIREBALL, 30, MON_SPELL_MAGICAL },
       { SPELL_MALMUTATE, 20, MON_SPELL_MAGICAL },
      }
    },

    // (';') Walking Tomes.
    { MST_WALKING_TOME,
      {
       { SPELL_CONJURE_LIVING_SPELLS, 60, MON_SPELL_WIZARD },
      }
    },

    // ---------------------
    // Uniques' spellbooks
    // ---------------------

    // ('a') "Ancients".
    {  MST_ZENATA,
      {
       { SPELL_RESONANCE_STRIKE, 40, MON_SPELL_WIZARD },
       { SPELL_SHEZAS_DANCE, 30, MON_SPELL_WIZARD },
      }
    },

    // ('c') Centaurs and such.
    {  MST_NESSOS,
      {
       { SPELL_BLINK_RANGE, 30, MON_SPELL_WIZARD },
       { SPELL_HASTE, 30, MON_SPELL_WIZARD },
      }
    },

    // ('d') Draconians.
    { MST_BAI_SUZHEN,
      {
        { SPELL_SUMMON_HYDRA, 40, MON_SPELL_WIZARD },
      }
    },

    // ('e') Elves.
    {  MST_DOWAN,
      {
       { SPELL_THROW_FLAME, 10, MON_SPELL_WIZARD },
       { SPELL_THROW_FROST, 10, MON_SPELL_WIZARD },
       { SPELL_BLINK, 10, MON_SPELL_WIZARD },
       { SPELL_CORONA, 10, MON_SPELL_WIZARD },
       { SPELL_HASTE_OTHER, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_FANNAR,
      {
       { SPELL_BOLT_OF_COLD, 10, MON_SPELL_WIZARD },
       { SPELL_OZOCUBUS_REFRIGERATION, 20, MON_SPELL_WIZARD },
       { SPELL_SUMMON_ICE_BEAST, 20, MON_SPELL_WIZARD },
       { SPELL_BLINK, 10, MON_SPELL_WIZARD | MON_SPELL_EMERGENCY },
      }
    },

    // ('g') Small humanoids.
    {  MST_ROBIN,
      {
        { SPELL_BATTLECRY, 40, MON_SPELL_VOCAL },
        { SPELL_THROW_ALLY, 80, MON_SPELL_NATURAL }
      }
    },

    {  MST_JORGRUN,
      {
       { SPELL_LRD, 20, MON_SPELL_WIZARD },
       { SPELL_GRASPING_ROOTS, 20, MON_SPELL_WIZARD },
       { SPELL_IRON_SHOT, 10, MON_SPELL_WIZARD },
       { SPELL_PETRIFY, 10, MON_SPELL_WIZARD },
       { SPELL_DIG, 10, MON_SPELL_WIZARD },
      }
    },

    // ('h') Carnivorous quadrupeds.
    {  MST_NATASHA,
      {
       { SPELL_MAGIC_DART, 10, MON_SPELL_WIZARD },
       { SPELL_CALL_IMP, 10, MON_SPELL_WIZARD },
       { SPELL_SLOW, 10, MON_SPELL_WIZARD },
      }
    },

    // ('i') Spriggans.
    { MST_THE_ENCHANTRESS,
      {
       { SPELL_SLOW, 10, MON_SPELL_WIZARD },
       { SPELL_DIMENSION_ANCHOR, 10, MON_SPELL_WIZARD },
       { SPELL_MASS_CONFUSION, 10, MON_SPELL_WIZARD },
       { SPELL_STRIP_WILLPOWER, 10, MON_SPELL_WIZARD },
       { SPELL_HASTE, 10, MON_SPELL_WIZARD },
       { SPELL_REPEL_MISSILES, 10, MON_SPELL_WIZARD },
      }
    },

    // ('m') Merfolk and friends.
    { MST_ILSUIW,
      {
       { SPELL_THROW_ICICLE, 10, MON_SPELL_WIZARD },
       { SPELL_WATER_ELEMENTALS, 20, MON_SPELL_WIZARD },
       { SPELL_CALL_TIDE, 10, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD },
       { SPELL_BLINK, 10, MON_SPELL_WIZARD },
      }
    },

    // ('o') Orcs.
    {  MST_BLORK,
      {
       { SPELL_CANTRIP, 10, MON_SPELL_WIZARD },
       { SPELL_MAGIC_DART, 10, MON_SPELL_WIZARD },
       { SPELL_THROW_FROST, 10, MON_SPELL_WIZARD },
       { SPELL_THROW_FLAME, 10, MON_SPELL_WIZARD },
       { SPELL_HASTE, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_NERGALLE,
      {
       { SPELL_BOLT_OF_DRAINING, 10, MON_SPELL_WIZARD },
       { SPELL_SUMMON_SPECTRAL_ORCS, 20, MON_SPELL_WIZARD },
       { SPELL_HASTE_OTHER, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_SAINT_ROKA,
      {
       { SPELL_BATTLECRY, 100, MON_SPELL_VOCAL },
       { SPELL_SMITING, 60, MON_SPELL_PRIEST },
      }
    },

    // ('w') Slugs and worms.
    {  MST_GASTRONOK,
      {
       { SPELL_CANTRIP, 10, MON_SPELL_WIZARD },
       { SPELL_AIRSTRIKE, 20, MON_SPELL_WIZARD },
       { SPELL_SUMMON_SMALL_MAMMAL, 10, MON_SPELL_WIZARD },
       { SPELL_SLOW, 10, MON_SPELL_WIZARD },
       { SPELL_SPRINT, 10, MON_SPELL_WIZARD },
      }
    },

    // ('z') Corporeal undead.
    {  MST_MURRAY,
      {
       { SPELL_SYMBOL_OF_TORMENT, 10, MON_SPELL_MAGICAL | MON_SPELL_NOISY },
       { SPELL_SUMMON_UNDEAD, 30, MON_SPELL_MAGICAL | MON_SPELL_NOISY },
       { SPELL_BLACK_MARK, 20, MON_SPELL_MAGICAL | MON_SPELL_NOISY },
      }
    },

    // ('A') Angels.
    {  MST_MENNAS,
      {
       { SPELL_MASS_CONFUSION, 20, MON_SPELL_WIZARD },
       { SPELL_SILENCE, 20, MON_SPELL_WIZARD },
       { SPELL_MINOR_HEALING, 30, MON_SPELL_WIZARD },
      }
    },

    // ('C') Giants.
    {  MST_POLYPHEMUS,
      {
        { SPELL_THROW_ALLY, 40, MON_SPELL_NATURAL }
      }
    },

    // ('D') Dragons.
    {  MST_XTAHUA,
      {
       { SPELL_SEARING_BREATH, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH
                                    | MON_SPELL_NOISY },
       { SPELL_PARALYSE, 20, MON_SPELL_VOCAL },
      }
    },

    { MST_BAI_SUZHEN_DRAGON,
      {
        { SPELL_PRIMAL_WAVE, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH },
      }
    },

    {  MST_SERPENT_OF_HELL_GEH,
      {
       { SPELL_SERPENT_OF_HELL_GEH_BREATH, 40,
           MON_SPELL_NATURAL | MON_SPELL_BREATH | MON_SPELL_NOISY },
       { SPELL_SUMMON_DRAGON, 30, MON_SPELL_MAGICAL },
      }
    },

    {  MST_SERPENT_OF_HELL_COC,
      {
       { SPELL_SERPENT_OF_HELL_COC_BREATH, 40,
         MON_SPELL_NATURAL | MON_SPELL_BREATH | MON_SPELL_NOISY },
       { SPELL_SUMMON_DRAGON, 30, MON_SPELL_MAGICAL },
      }
    },

    {  MST_SERPENT_OF_HELL_DIS,
      {
       { SPELL_SERPENT_OF_HELL_DIS_BREATH, 40,
         MON_SPELL_NATURAL | MON_SPELL_BREATH | MON_SPELL_NOISY },
       { SPELL_SUMMON_DRAGON, 30, MON_SPELL_MAGICAL },
      }
    },

    {  MST_SERPENT_OF_HELL_TAR,
      {
       { SPELL_SERPENT_OF_HELL_TAR_BREATH, 40,
         MON_SPELL_NATURAL | MON_SPELL_BREATH | MON_SPELL_NOISY },
       { SPELL_SUMMON_DRAGON, 30, MON_SPELL_MAGICAL },
      }
    },

    // ('H') Hybrids.
    {  MST_ARACHNE,
      {
       { SPELL_VENOM_BOLT, 10, MON_SPELL_WIZARD },
       { SPELL_POISON_ARROW, 10, MON_SPELL_WIZARD },
       { SPELL_ENSNARE, 30, MON_SPELL_WIZARD },
       { SPELL_BLINK, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_ASTERION,
      {
       { SPELL_MAJOR_DESTRUCTION, 10, MON_SPELL_PRIEST },
       { SPELL_GREATER_SERVANT_MAKHLEB, 20, MON_SPELL_PRIEST },
       { SPELL_HASTE, 30, MON_SPELL_PRIEST },
      }
    },

    // ('J') Jellies.
    { MST_ENDOPLASM,
      {
        { SPELL_FREEZE, 60, MON_SPELL_NATURAL },
      }
    },

    {  MST_DISSOLUTION,
      {
       { SPELL_SUMMON_EYEBALLS, 60, MON_SPELL_PRIEST },
      }
    },

    // ('K') Kobolds.
    {  MST_SONJA,
      {
       { SPELL_BLINK, 30, MON_SPELL_WIZARD },
      }
    },

    // ('L') Liches.
    {  MST_BORIS,
      {
       { SPELL_BOLT_OF_COLD, 10, MON_SPELL_WIZARD },
       { SPELL_IRON_SHOT, 10, MON_SPELL_WIZARD },
       { SPELL_IOOD, 10, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 30, MON_SPELL_WIZARD },
      }
    },

    // ('M') Mummies.
    {  MST_MENKAURE,
      {
       { SPELL_PAIN, 20, MON_SPELL_WIZARD },
       { SPELL_HASTE, 20, MON_SPELL_WIZARD },
       { SPELL_SYMBOL_OF_TORMENT, 20, MON_SPELL_WIZARD },
      }
    },

    {  MST_KHUFU,
      {
       { SPELL_SMITING, 20, MON_SPELL_WIZARD },
       { SPELL_SYMBOL_OF_TORMENT, 10, MON_SPELL_WIZARD },
       { SPELL_SUMMON_EMPEROR_SCORPIONS, 10, MON_SPELL_WIZARD },
       { SPELL_SUMMON_SCARABS, 10, MON_SPELL_WIZARD },
       { SPELL_BLINK, 10, MON_SPELL_WIZARD | MON_SPELL_EMERGENCY },
      }
    },

    // ('N') Naga.
    {  MST_VASHNIA,
      {
       { SPELL_SPIT_POISON, 60, MON_SPELL_NATURAL | MON_SPELL_BREATH },
       { SPELL_PORTAL_PROJECTILE, 20, MON_SPELL_WIZARD },
       { SPELL_BLINK_OTHER, 10, MON_SPELL_WIZARD | MON_SPELL_EMERGENCY },
       { SPELL_BLINK_ALLIES_AWAY, 20, MON_SPELL_WIZARD },
       { SPELL_BLINK_AWAY, 10, MON_SPELL_WIZARD },
      }
    },

    // ('O') Lodul.
    {  MST_LODUL,
      {
       { SPELL_CALL_DOWN_LIGHTNING, 40, MON_SPELL_MAGICAL },
      }
    },

    // ('Q') Tengu.
    {  MST_SOJOBO,
      {
       { SPELL_WIND_BLAST, 30, MON_SPELL_WIZARD },
       { SPELL_LIGHTNING_BOLT, 10, MON_SPELL_WIZARD },
       { SPELL_AIRSTRIKE, 10, MON_SPELL_WIZARD },
       { SPELL_AIR_ELEMENTALS, 20, MON_SPELL_WIZARD },
       { SPELL_REPEL_MISSILES, 10, MON_SPELL_WIZARD },
      }
    },

    // ('R') Spiritual beings.
    {  MST_AZRAEL,
      {
       { SPELL_BOLT_OF_FIRE, 10, MON_SPELL_MAGICAL },
       { SPELL_STICKY_FLAME_RANGE, 10, MON_SPELL_MAGICAL },
       { SPELL_FIREBALL, 10, MON_SPELL_MAGICAL },
       { SPELL_HURL_DAMNATION, 20, MON_SPELL_MAGICAL },
       { SPELL_CALL_DOWN_DAMNATION, 10, MON_SPELL_MAGICAL },
      }
    },

    {  MST_MARA,
      {
       { SPELL_BOLT_OF_FIRE, 10, MON_SPELL_MAGICAL },
       { SPELL_FAKE_MARA_SUMMON, 10, MON_SPELL_MAGICAL },
       { SPELL_SUMMON_ILLUSION, 20, MON_SPELL_MAGICAL },
       { SPELL_BLINK, 30, MON_SPELL_MAGICAL },
      }
    },

    // ('S') Snakes.
    {  MST_AIZUL,
      {
       { SPELL_VENOM_BOLT, 30, MON_SPELL_WIZARD },
       { SPELL_POISON_ARROW, 20, MON_SPELL_WIZARD },
       { SPELL_SLEEP, 20, MON_SPELL_WIZARD },
      }
    },

    // ('T') Trolls.
    {  MST_SNORG,
      {
       { SPELL_BERSERKER_RAGE, 30, MON_SPELL_NATURAL },
      }
    },

    {  MST_MOON_TROLL,
      {
       { SPELL_CORROSIVE_BOLT, 60, MON_SPELL_WIZARD },
      }
    },

    // ('V') Vampires.
    {  MST_JORY,
      {
       { SPELL_VAMPIRIC_DRAINING, 40, MON_SPELL_WIZARD },
       { SPELL_LEHUDIBS_CRYSTAL_SPEAR, 20, MON_SPELL_WIZARD },
       { SPELL_MESMERISE, 20, MON_SPELL_WIZARD },
       { SPELL_BLINK_CLOSE, 20, MON_SPELL_WIZARD },
      }
    },

    // ('5') Lesser demons.
    {  MST_GRINDER,
      {
       { SPELL_PAIN, 20, MON_SPELL_MAGICAL },
       { SPELL_PARALYSE, 20, MON_SPELL_MAGICAL },
       { SPELL_BLINK, 20, MON_SPELL_MAGICAL },
      }
    },

    // ('1') Greater demons.
    {  MST_IGNACIO,
      {
       { SPELL_AGONY_RANGE, 20, MON_SPELL_MAGICAL },
       { SPELL_HASTE, 40, MON_SPELL_MAGICAL },
      }
    },

    // ('6') Demonspawn.
    {  MST_AMAEMON,
      {
        { SPELL_CONCENTRATE_VENOM, 30, MON_SPELL_WIZARD },
        { SPELL_SUMMON_SCORPIONS, 10, MON_SPELL_WIZARD },
      }
    },

    // ('8') Statues.
    { MST_ROXANNE,
      {
       { SPELL_BOLT_OF_MAGMA, 10, MON_SPELL_WIZARD },
       { SPELL_IRON_SHOT, 10, MON_SPELL_WIZARD },
       { SPELL_LEHUDIBS_CRYSTAL_SPEAR, 20, MON_SPELL_WIZARD },
       { SPELL_BLINK_OTHER_CLOSE, 10, MON_SPELL_WIZARD },
      }
    },

    // ('9') Mobile constructs.
    {  MST_VV,
      {
       { SPELL_ERUPTION, 20, MON_SPELL_MAGICAL },
       { SPELL_PYROCLASTIC_SURGE, 60, MON_SPELL_MAGICAL },
       { SPELL_CREEPING_FROST, 20, MON_SPELL_MAGICAL },
      }
    },

    // ('@') Human (or close enough to) uniques.
    {  MST_JESSICA,
      {
       { SPELL_PAIN, 20, MON_SPELL_WIZARD },
       { SPELL_SLOW, 10, MON_SPELL_WIZARD },
       { SPELL_HASTE, 10, MON_SPELL_WIZARD },
       { SPELL_BLINK, 20, MON_SPELL_WIZARD },
      }
    },

    { MST_EUSTACHIO,
      {
       { SPELL_SUMMON_SMALL_MAMMAL, 10, MON_SPELL_WIZARD },
       { SPELL_SUMMON_MINOR_DEMON, 20, MON_SPELL_WIZARD },
       { SPELL_BLINK, 30, MON_SPELL_WIZARD },
      }
    },

    {  MST_ERICA,
      {
       { SPELL_VENOM_BOLT, 10, MON_SPELL_WIZARD },
       { SPELL_FORCE_LANCE, 10, MON_SPELL_WIZARD },
       { SPELL_CONFUSE, 10, MON_SPELL_WIZARD },
       { SPELL_SLOW, 10, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_MAURICE,
      {
       { SPELL_SPRINT, 20, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD },
       { SPELL_BLINK, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_HAROLD,
      {
       { SPELL_HARPOON_SHOT, 20, MON_SPELL_WIZARD },
       { SPELL_SENTINEL_MARK, 20, MON_SPELL_WIZARD },
      }
    },

    {  MST_JOSEPHINE,
      {
       { SPELL_VAMPIRIC_DRAINING, 10, MON_SPELL_WIZARD },
       { SPELL_GHOSTLY_FIREBALL, 30, MON_SPELL_WIZARD },
       { SPELL_DISPEL_UNDEAD_RANGE, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_JOSEPHINA,
      {
       { SPELL_SERACFALL, 40, MON_SPELL_WIZARD },
       { SPELL_GHOSTLY_FIREBALL, 20, MON_SPELL_WIZARD },
       { SPELL_BIND_SOULS, 20, MON_SPELL_WIZARD },
       { SPELL_FLASH_FREEZE, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_RUPERT,
      {
       { SPELL_PARALYSE, 20, MON_SPELL_VOCAL },
       { SPELL_CONFUSE, 20, MON_SPELL_VOCAL },
       { SPELL_BERSERKER_RAGE, 30, MON_SPELL_NATURAL },
      }
    },

    {  MST_LOUISE,
      {
       { SPELL_STONE_ARROW, 10, MON_SPELL_WIZARD },
       { SPELL_LIGHTNING_BOLT, 10, MON_SPELL_WIZARD },
       { SPELL_BANISHMENT, 10, MON_SPELL_WIZARD },
       { SPELL_BLINK, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_FRANCES,
      {
       { SPELL_THROW_ICICLE, 10, MON_SPELL_WIZARD },
       { SPELL_IRON_SHOT, 10, MON_SPELL_WIZARD },
       { SPELL_SUMMON_DEMON, 20, MON_SPELL_WIZARD },
       { SPELL_HASTE, 20, MON_SPELL_WIZARD },
      }
    },

    {  MST_KIRKE,
      {
       { SPELL_PORKALATOR, 20, MON_SPELL_WIZARD },
       { SPELL_SLOW, 10, MON_SPELL_WIZARD },
       { SPELL_MONSTROUS_MENAGERIE, 10, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD | MON_SPELL_EMERGENCY },
      }
    },

    {  MST_DONALD,
      {
       { SPELL_MIGHT, 30, MON_SPELL_WIZARD },
       { SPELL_HASTE, 30, MON_SPELL_WIZARD },
      }
    },

    {  MST_HELLBINDER,
      {
       { SPELL_HURL_DAMNATION, 10, MON_SPELL_WIZARD },
       { SPELL_SUMMON_GREATER_DEMON, 30, MON_SPELL_WIZARD },
       { SPELL_SUMMON_DEMON, 10, MON_SPELL_WIZARD },
       { SPELL_HASTE, 10, MON_SPELL_WIZARD },
       { SPELL_BLINK_AWAY, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_CLOUD_MAGE,
      {
       { SPELL_AIRSTRIKE, 20, MON_SPELL_WIZARD },
       { SPELL_POLAR_VORTEX, 40, MON_SPELL_WIZARD },
      }
    },

    {  MST_HEADMASTER,
      {
       { SPELL_SUMMON_MANA_VIPER, 20, MON_SPELL_WIZARD },
       { SPELL_DIMENSION_ANCHOR, 10, MON_SPELL_WIZARD },
       { SPELL_BLINK_CLOSE, 20, MON_SPELL_WIZARD },
       { SPELL_ENERGY_BOLT, 10, MON_SPELL_WIZARD },
      }
    },

    {  MST_NIKOLA,
      {
       { SPELL_LIGHTNING_BOLT, 10, MON_SPELL_WIZARD },
       { SPELL_CHAIN_LIGHTNING, 20, MON_SPELL_WIZARD },
       { SPELL_BLINK, 20, MON_SPELL_WIZARD },
      }
    },

    {  MST_MAGGIE,
      {
       { SPELL_BOLT_OF_FIRE, 20, MON_SPELL_WIZARD },
       { SPELL_MESMERISE, 30, MON_SPELL_WIZARD },
      }
    },

    {  MST_MARGERY,
      {
       { SPELL_BOLT_OF_FIRE, 30, MON_SPELL_WIZARD },
       { SPELL_FIREBALL, 40, MON_SPELL_WIZARD },
      }
    },

    {  MST_FREDERICK,
      {
       { SPELL_FORCE_LANCE, 10, MON_SPELL_WIZARD },
       { SPELL_BOLT_OF_COLD, 10, MON_SPELL_WIZARD },
       { SPELL_IRON_SHOT, 10, MON_SPELL_WIZARD },
       { SPELL_SPELLFORGED_SERVITOR, 30, MON_SPELL_WIZARD },
      }
    },

    {  MST_PSYCHE,
      {
       { SPELL_CANTRIP, 10, MON_SPELL_WIZARD },
       { SPELL_POLYMORPH, 10, MON_SPELL_WIZARD },
       { SPELL_CHAIN_OF_CHAOS, 30, MON_SPELL_WIZARD },
       { SPELL_INVISIBILITY, 10, MON_SPELL_WIZARD },
      }
    },
    // ('&', mostly) Demon lords.
    {  MST_GERYON,
      {
       { SPELL_SUMMON_HELL_BEAST, 60, MON_SPELL_VOCAL },
      }
    },

    {  MST_ASMODEUS,
      {
       { SPELL_BOLT_OF_FIRE, 20, MON_SPELL_MAGICAL },
       { SPELL_HURL_DAMNATION, 20, MON_SPELL_MAGICAL },
       { SPELL_FIRE_SUMMON, 20, MON_SPELL_MAGICAL },
      }
    },

    {  MST_ANTAEUS,
      {
       { SPELL_LIGHTNING_BOLT, 20, MON_SPELL_MAGICAL },
       { SPELL_FLASH_FREEZE, 40, MON_SPELL_MAGICAL },
      }
    },

    {  MST_ERESHKIGAL,
      {
       { SPELL_BOLT_OF_COLD, 10, MON_SPELL_MAGICAL },
       { SPELL_SYMBOL_OF_TORMENT, 10, MON_SPELL_MAGICAL },
       { SPELL_SUMMON_TZITZIMITL, 10, MON_SPELL_MAGICAL },
       { SPELL_PARALYSE, 10, MON_SPELL_MAGICAL },
       { SPELL_SILENCE, 10, MON_SPELL_MAGICAL },
       { SPELL_MAJOR_HEALING, 10, MON_SPELL_MAGICAL | MON_SPELL_EMERGENCY },
      }
    },

    {  MST_DISPATER,
      {
       { SPELL_IRON_SHOT, 10, MON_SPELL_MAGICAL },
       { SPELL_LEHUDIBS_CRYSTAL_SPEAR, 10, MON_SPELL_MAGICAL },
       { SPELL_HURL_DAMNATION, 10, MON_SPELL_MAGICAL },
       { SPELL_SUMMON_HELL_SENTINEL, 30, MON_SPELL_MAGICAL },
      }
    },

    {  MST_MNOLEG,
      {
       { SPELL_MALIGN_GATEWAY, 30, MON_SPELL_MAGICAL },
       { SPELL_SUMMON_EYEBALLS, 10, MON_SPELL_MAGICAL },
       { SPELL_SUMMON_HORRIBLE_THINGS, 30, MON_SPELL_MAGICAL },
       { SPELL_CALL_OF_CHAOS, 30, MON_SPELL_MAGICAL },
       { SPELL_DIG, 10, MON_SPELL_MAGICAL },
      }
    },

    {  MST_LOM_LOBON,
      {
       { SPELL_CONJURE_BALL_LIGHTNING, 30, MON_SPELL_MAGICAL },
       { SPELL_GLACIATE, 30, MON_SPELL_MAGICAL },
       { SPELL_POLAR_VORTEX, 60, MON_SPELL_MAGICAL },
       { SPELL_MAJOR_HEALING, 30, MON_SPELL_MAGICAL },
       { SPELL_BLINK_RANGE, 30, MON_SPELL_MAGICAL },
      }
    },

    {  MST_CEREBOV,
      {
       { SPELL_IRON_SHOT, 10, MON_SPELL_MAGICAL },
       { SPELL_FIRE_STORM, 10, MON_SPELL_MAGICAL },
       { SPELL_FIRE_SUMMON, 10, MON_SPELL_MAGICAL },
       { SPELL_HASTE, 40, MON_SPELL_MAGICAL },
      }
    },

    {  MST_GLOORX_VLOQ,
      {
       { SPELL_POISON_ARROW, 20, MON_SPELL_MAGICAL },
       { SPELL_MIASMA_BREATH, 20, MON_SPELL_MAGICAL },
       { SPELL_SYMBOL_OF_TORMENT, 20, MON_SPELL_MAGICAL },
       { SPELL_DISPEL_UNDEAD_RANGE, 20, MON_SPELL_MAGICAL },
       { SPELL_SUMMON_EXECUTIONERS, 40, MON_SPELL_MAGICAL },
      }
    },

    // A monster that doesn't show up anywhere, for Arena testing.
    {  MST_TEST_SPAWNER,
      {
       { SPELL_SHADOW_CREATURES, 100, MON_SPELL_MAGICAL },
       { SPELL_PLANEREND, 60, MON_SPELL_MAGICAL },
       { SPELL_PHANTOM_MIRROR, 40, MON_SPELL_MAGICAL },
      }
    },
};
