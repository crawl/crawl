...
spret_type cast_dragon_call(int pow, bool fail)
{
	...
	mpr("당신은 용족의 왕국에 소환 요청을 했고, 용 무리가 울음으로 답했다!");
...
spret_type cast_haunt(int pow, const coord_def& where, god_type god, bool fail)
{
	...
	{
		mpr("당신에게 적대감을 표하지 않는 존재를 위협할 수는 없다.");
        return SPRET_ABORT;
    }
...
spret_type cast_aura_of_abjuration(int pow, bool fail)
{
    ...
    if (!you.duration[DUR_ABJURATION_AURA])
        mpr("당신은 당신 근처의 존재들을 송환시키기 시작했다!");
    else
        mpr("당신은 송환의 오라를 연장시켰다.");
    ...
    if (!killed)
    {
        if (agent && agent->is_player())
        {
            if (you.can_see(*mons))
            {
                ...
                {
					mpr("당신의 전투 구체가 마지막 에너지를 뿜어내곤 증발했다.");
				}
				else
					mpr("당신의 전투 구체가 불안정하게 흔들리더니 결속력을 잃었다.");
			}
			else
				mpr("당신과 전투 구체 간의 결속이 약해진 것을 느꼈다.");
        }
...
void end_spectral_weapon(monster* mons, bool killed, bool quiet)
{
    ...
    if (!quiet)
    {
        if (you.can_see(*mons))
        {
			...
			mpr("당신과 떠다니는 무기 간의 결속이 약해진 것을 느꼈다.");
    }
