...
int get_jinxsprite_summon_power(const actor& agent)
{
    ...
    // Gain a strong powerboost if this cast of jinxbite has not produced a
    // single sprite yet (to avoid the bad feels of never getting anything)
    if (agent.is_player() && you.props.exists(JINXBITE_FIRST_SHOT_KEY))
        pow *= 2;

    return pow * 3 / 2;
}

bool maybe_make_jinxsprite(const actor& agent, const actor& victim, string source_name)
{
    // Try to find a spot near the victim
    coord_def pos;
    for (fair_adjacent_iterator ai(victim.pos()); ai; ++ai)
    {
        if (cell_is_solid(*ai) || monster_at(*ai) || !(agent.see_cell(*ai)))
            continue;
    ...
    else
    {
        ...
        // to the player that this is a Will check, also)
    ...
    const auto att = agent.is_player() ? BEH_FRIENDLY
                                       : SAME_ATTITUDE(agent.as_monster());
    mgen_data sprite_data = mgen_data(MONS_JINXSPRITE, att, pos, MHITNOT,
            MG_FORCE_PLACE | MG_DONT_COME).set_summoned(&agent, 0, SPELL_JINXBITE);
    sprite_data.hd = 2 + div_rand_round(calc_spell_power(SPELL_JINXBITE), 25);

    if (monster* sprite = create_monster(sprite_data))
    {
        mprf("A giggling sprite leaps out of %s.", source_name.c_str());

        sprite->add_ench(ENCH_SHORT_LIVED);
        sprite->steps_remaining = you.current_vision + 2;

        //Find a target for these sprites. Randomized, but favoring further enemies
        actor* foe = nullptr;
        for (distance_iterator di(sprite->pos(), false, true, LOS_DEFAULT_RANGE); di; ++di)
        {
            actor* targ = actor_at(*di);
        ...
        if (foe)
        {
            sprite->foe = foe->mindex();
            sprite->target = foe->pos();
            sprite->behaviour = BEH_SEEK;
        }

        // Drain some duration from the effect every time we spawn a sprite
        // (And mark that we have summoned at least one from this spellcast)
        you.duration[DUR_JINXBITE] -= 4;
        if (you.duration[DUR_JINXBITE] < 1)
            you.duration[DUR_JINXBITE] = 1;

        you.props.erase(JINXBITE_FIRST_SHOT_KEY);
...
spret cast_jinxbite(int pow, bool fail)
{
    ...
    mprf("You beckon %s vexing sprites into your weapons.",
         you.duration[DUR_JINXBITE] ? "more" : "some");

    int dur = random_range(8, 12) + div_rand_round(pow, 5);

    you.increase_duration(DUR_JINXBITE, dur);
    you.props[JINXBITE_FIRST_SHOT_KEY] = true;
