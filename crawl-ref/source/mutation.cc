...
static bool _is_existing_upgradeable_mutation(mutation_def mutdef)
{
    return you.mutation[mutdef.mutation] != 0 && you.mutation[mutdef.mutation] < mutdef.levels;
}

static mutation_type _get_mut_with_use(mutflag mt, bool prefer_existing)
{
    dprf("_get_mut_with_use(prefer_existing=%d)", prefer_existing);
    if (prefer_existing)
    {
        ...
        int existing_tweight = 0;
        for (const mutation_def &mutdef : mut_data)
        {
            if (_is_existing_upgradeable_mutation(mutdef))
                existing_tweight += mutdef.weight;
        }

        int existing_cweight = random2(existing_tweight);

        for (const mutation_def &mutdef : mut_data)
        {
            if (_is_existing_upgradeable_mutation(mutdef))
            {
                // xxx this could underflow, I guess
                existing_cweight -= _mut_weight(mutdef, mt);
                if (existing_cweight >= 0 || !_mut_has_use(mutdef, mt))
                    continue;
                dprf("Chose mutation %s", mutdef.short_desc);
                return mutdef.mutation;
            }
        }
        dprf("Didn't find an existing mutation.");
    }
