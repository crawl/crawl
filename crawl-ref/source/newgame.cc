...
UISizeReq UINewGameMenu::_get_preferred_size(Direction dim, int prosp_width)
{
    UISizeReq ret;
    if (!dim)
        ret = { 80, 80 };
...
UISizeReq UIPrecisionMenuWrapper::_get_preferred_size(Direction dim, int prosp_width)
{
    UISizeReq ret;
    if (!dim)
        ret = { w, w };
