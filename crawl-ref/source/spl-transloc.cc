...
#include "directn.h"
#include "dungeon.h"
#include "god-abil.h" // fedhas_passthrough for centaur charge
#include "item-prop.h"
#include "items.h"
...
#include "losglobal.h"
#include "losparam.h"
#include "melee-attack.h" // centaur charge
#include "message.h"
#include "mon-behv.h"
...
static bool _check_charge_through(coord_def pos) {
    ...
    if (!check_moveto_terrain(pos, "charge") || !check_moveto_trap(pos, "charge"))
        return false;
...
bool _find_charge_target(coord_def &target, int max_range, targeter *hitfunc) {
    while (true) {
        ...
        if (crawl_state.seen_hups)
        {
            mpr("Cancelling charge due to HUP.");
            return false;
        }
        ...
        if (attacking) {
            ...
            {
                mprf("You're already next to %s!", target_mons->name(DESC_THE).c_str());
                continue;
            }
...
spret centaur_charge(bool fail)
{
    ...
    while (ray.advance()
        ...
        if (you.pos() != ray.pos()) { // trap really went off!
            you.increase_duration(DUR_NO_CHARGE, 12 + random2(13));
    ...
    if (target_mons || grid_distance(you.pos(), ray.pos()) == charge_range)
        ray.regress();
    ...
    if (ray.pos() != orig_pos) {
        ...
        if (you.pos() != ray.pos()) // tornado nonsense
            return spret::success; // of a sort
            // Don't give dur_no_charge here - tornados are silly
    }
    ...
    if (target_mons)
    {
        ...
        charge_atk.charge_dist = grid_distance(initial_pos, you.pos());
        charge_atk.attack();
    } else {
        you.increase_duration(DUR_NO_CHARGE, 12 + random2(13));
    }
