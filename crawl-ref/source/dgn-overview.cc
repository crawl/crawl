...
static int _prompt_annotate_branch(level_id lid)
{
    ...
    for (branch_type br : brs)
    {
        ...
        line += make_stringf("(%c) %-14s ", branches[br].travel_shortcut,
...
void do_annotate()
{
    ...
    else
    {
        clear_messages();
        const string prompt =
            make_stringf("What level of %s? ", branches[branch].longname);
...
void annotate_level(level_id li)
{
    ...
    {
        mprf(MSGCH_PROMPT,
             "Current level annotation: <lightgrey>%s</lightgrey>",
             old.c_str());
    }

    const string prompt =
        "New annotation for " + li.describe() + " (include '!' for warning): ";
